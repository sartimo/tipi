...
context 'API' do
  context 'Load' do
    test 'should load input file' do
      sample_input_path = fixture_path 'sample.tipi'
      doc = File.open(sample_input_path, Tipi::FILE_READ_MODE) {|file| Tipi.load file, safe: Tipi::SafeMode::SAFE }
      ...
      assert_equal '.tipi', doc.attr('docfilesuffix')
    end

    test 'should load input file from filename' do
      sample_input_path = fixture_path 'sample.tipi'
      doc = Tipi.load_file sample_input_path, safe: Tipi::SafeMode::SAFE
      assert_equal 'Document Title', doc.doctitle
      assert_equal File.expand_path(sample_input_path), doc.attr('docfile')
      assert_equal File.expand_path(File.dirname(sample_input_path)), doc.attr('docdir')
      assert_equal '.tipi', doc.attr('docfilesuffix')
    ...
    test 'should load input file from pathname' do
      sample_input_path = Pathname fixture_path 'sample.tipi'
      doc = Tipi.load_file sample_input_path, safe: :safe
      assert_equal 'Document Title', doc.doctitle
    ...
    test 'should load input file with alternate file extension' do
      sample_input_path = fixture_path 'sample-alt-extension.tipi'
      doc = Tipi.load_file sample_input_path, safe: :safe
      assert_equal 'Document Title', doc.doctitle
      assert_equal File.expand_path(sample_input_path), doc.attr('docfile')
      assert_equal File.expand_path(File.dirname(sample_input_path)), doc.attr('docdir')
    ...
    test 'should coerce encoding of file to UTF-8' do
      ...
      begin
        ...
        input_path = fixture_path 'encoding.tipi'
        Encoding.default_external = Encoding.default_internal = Encoding::IBM437
        output = Tipi.convert_file input_path, to_file: false, safe: :safe
    ...
    test 'should not load file with unrecognized encoding' do
      begin
        tmp_input = Tempfile.new %w(test- .tipi), encoding: Encoding::IBM437
        # NOTE using a character whose code differs between UTF-8 and IBM437
        tmp_input.write %(ƒ\n)
        tmp_input.close
        exception = assert_raises ArgumentError do
          Tipi.load_file tmp_input.path, safe: :safe
        end
        expected_message = 'Failed to load Tipi document - source is either binary or contains invalid Unicode data'
    ...
    test 'should not load invalid file' do
      sample_input_path = fixture_path 'hello-tipi.pdf'
      exception = assert_raises ArgumentError do
        Tipi.load_file sample_input_path, safe: Tipi::SafeMode::SAFE
      end
      expected_message = 'Failed to load Tipi document - source is either binary or contains invalid Unicode data'
      assert_include expected_message, exception.message
    ...
    test 'should convert filename that contains non-ASCII characters independent of default encodings', unless: (jruby? && windows?) do
      ...
      begin
        $VERBOSE = nil # disable warnings since we have to modify constants
        tmp_input = Tempfile.new %w(test-ＵＴＦ８- .tipi)
        tmp_input.write %(ＵＴＦ８\n)
        tmp_input.close
        Encoding.default_external = Encoding.default_internal = Encoding::IBM437
        tmp_output = tmp_input.path.sub '.tipi', '.html'
        Tipi.convert_file tmp_input.path, safe: :safe, attributes: 'linkcss !copycss'
    ...
    test 'should load input IO' do
      ...
      preamble
      EOS
      doc = Tipi.load input, safe: Tipi::SafeMode::SAFE
      assert_equal 'Document Title', doc.doctitle
    ...
    test 'should load input string array' do
      ...
      preamble
      EOS
      doc = Tipi.load input.lines, safe: Tipi::SafeMode::SAFE
      assert_equal 'Document Title', doc.doctitle
      refute doc.attr?('docfile')
      assert_equal doc.base_dir, doc.attr('docdir')
    end
    ...
    test 'should ignore :to_file option if value is truthy but not a string' do
      sample_input_path = fixture_path 'sample.tipi'
      doc = Tipi.load_file sample_input_path, safe: :safe, to_file: true
      refute_nil doc
      assert_equal 'Document Title', doc.doctitle
      assert_equal '.html', (doc.attr 'outfilesuffix')
      assert_equal doc.convert, (Tipi.convert_file sample_input_path, safe: :safe, to_file: false)
    end

    test 'should set outfilesuffix attribute to file extension of value of :to_file option if value is a string' do
      sample_input_path = fixture_path 'sample.tipi'
      doc = Tipi.load_file sample_input_path, safe: :safe, to_file: 'out.htm'
      refute_nil doc
      assert_equal 'Document Title', doc.doctitle
    ...
    test 'should accept attributes as array' do
      ...
      doc = Tipi.load 'text', attributes: %w(toc sectnums   source-highlighter=coderay idprefix	idseparator=-) # rubocop:disable Layout/SpaceInsideArrayPercentLiteral
    ...
    test 'should accept attributes as empty array' do
      doc = Tipi.load 'text', attributes: []
      assert_kind_of Hash, doc.attributes
    end

    test 'should accept attributes as string' do
      doc = Tipi.load 'text', attributes: %(toc sectnums\nsource-highlighter=coderay\nidprefix\nidseparator=-)
      assert_kind_of Hash, doc.attributes
      assert doc.attr?('toc')
      assert_equal '', doc.attr('toc')
      assert doc.attr?('sectnums')
    ...
    test 'should accept values containing spaces in attributes string' do
      doc = Tipi.load 'text', attributes: %(idprefix idseparator=-   note-caption=Note\\ to\\\tself toc)
      assert_kind_of Hash, doc.attributes
      assert doc.attr?('idprefix')
      assert_equal '', doc.attr('idprefix')
      assert doc.attr?('idseparator')
    ...
    test 'should accept attributes as nil' do
      doc = Tipi.load 'text', attributes: nil
      assert_kind_of Hash, doc.attributes
    end
    ...
    test 'should accept attributes if hash like' do
      ...
      doc = Tipi.load 'text', attributes: Hashlike.new
      assert_kind_of Hash, doc.attributes
    ...
    test 'render method on node is aliased to convert method' do
      ...
      * list item
      ...
      inline = Tipi::Inline.new doc.blocks[0], :image, nil, type: 'image', target: 'tiger.png'
    ...
    test 'content method on Inline node is aliased to text method' do
      input = 'paragraph'
      doc = Tipi.load input
      inline = Tipi::Inline.new doc.blocks[0], :quoted, 'yo', type: :strong
    ...
    test 'should not modify options argument' do
      options = { safe: Tipi::SafeMode::SAFE }
      options.freeze
      sample_input_path = fixture_path 'sample.tipi'
      doc = Tipi.load_file sample_input_path, options
    ...
    test 'should track file and line information with blocks if sourcemap option is set' do
      doc = Tipi.load_file fixture_path('sample.tipi'), sourcemap: true

      refute_nil doc.source_location
      assert_equal 'sample.tipi', doc.file
      assert_equal 1, doc.lineno

      preamble = doc.blocks[0]
      refute_nil preamble.source_location
      assert_equal 'sample.tipi', preamble.file
      ...
      assert_equal 'sample.tipi', section_1.file
      ...
      assert_equal 'sample.tipi', section_2.file
    ...
    test 'should allow sourcemap option on document to be modified before document is parsed' do
      doc = Tipi.load_file fixture_path('sample.tipi'), parse: false
      doc.sourcemap = true
      refute doc.parsed?
      doc = doc.parse
      assert doc.parsed?
    ...
    test 'find_by should discover blocks inside Tipi table cells if traverse_documents selector option is true' do
      input = <<~'EOS'
    ...
    test 'find_by should return inner document of Tipi table cell if traverse_documents selector option is true' do
      input = <<~'EOS'
      |===
    ...
    test 'find_by should match table cells' do
      ...
      result = doc.find_by context: :table_cell, style: :tipi
      assert_equal 1, result.size
      assert_kind_of Tipi::Table::Cell, result[0]
      assert_equal :tipi, result[0].style
    ...
    test 'find_by should not crash if dlist entry does not have description' do
      ...
      refute_nil result
      assert_equal 3, result.size
      assert_kind_of Tipi::Document, result[0]
      assert_kind_of Tipi::List, result[1]
      assert_kind_of Tipi::ListItem, result[2]
    end
    ...
    test 'dlist item should always have two entries for terms and desc' do
      [
        ...
        dlist = (Tipi.load input).blocks[0]
        dlist.items.each do |item|
          assert_equal 2, item.size
          assert_kind_of Array, item[0]
          assert_kind_of Tipi::ListItem, item[1] if item[1]
        end
      end
    end

    test 'timings are recorded for each step when load and convert are called separately' do
      sample_input_path = fixture_path 'tipi_index.txt'
      (Tipi.load_file sample_input_path, timings: (timings = Tipi::Timings.new)).convert
    ...
    test 'can disable syntax highlighter by setting value to nil in :syntax_highlighters option' do
      doc = Tipi.load '', safe: :safe, syntax_highlighters: { 'coderay' => nil }, attributes: { 'source-highlighter' => 'coderay' }
    ...
    test 'can substitute a custom syntax highlighter factory instance using the :syntax_highlighter_factory option' do
      ...
      syntax_hl_factory = Tipi::SyntaxHighlighter::CustomFactory.new 'github' => (Tipi::SyntaxHighlighter.for 'html-pipeline')
      doc = Tipi.load input, safe: :safe, syntax_highlighter_factory: syntax_hl_factory, attributes: { 'source-highlighter' => 'github' }
      refute_nil doc.syntax_highlighter
      assert_kind_of Tipi::SyntaxHighlighter::HtmlPipelineAdapter, doc.syntax_highlighter
    ...
    test 'can substitute an extended syntax highlighter factory implementation using the :syntax_highlighters option' do
      ...
      syntax_hl_factory_class = Class.new do
        include Tipi::SyntaxHighlighter::DefaultFactory

        def for name
          super 'highlight.js'
        end
      end
      doc = Tipi.load input, safe: :safe, syntax_highlighter_factory: syntax_hl_factory_class.new, attributes: { 'source-highlighter' => 'coderay' }
      refute_nil doc.syntax_highlighter
      output = doc.convert
  ...
  context 'Convert' do
    test 'render_file is aliased to convert_file' do
      assert_equal Tipi.method(:convert_file), Tipi.method(:render_file)
    end

    test 'render is aliased to convert' do
      assert_equal Tipi.method(:convert), Tipi.method(:render)
    end
    ...
    test 'should convert source document to embedded document when header_footer is false' do
      ...
      [{ header_footer: false }, { header_footer: false, to_file: sample_output_path }].each do |opts|
        begin
          Tipi.convert_file sample_input_path, opts
          assert_path_exists sample_output_path
          output = File.read sample_output_path, mode: Tipi::FILE_READ_MODE
          refute_empty output
    ...
    test 'should convert source document to standalone document string when to_file is false and standalone is true' do
      ...
      output = Tipi.convert_file sample_input_path, standalone: true, to_file: false
      refute_empty output
    ...
    test 'should convert source document to standalone document string when to_file is false and header_footer is true' do
      ...
      output = Tipi.convert_file sample_input_path, header_footer: true, to_file: false
      refute_empty output
      assert_xpath '/html', output, 1
      assert_xpath '/html/head', output, 1
      assert_xpath '/html/body', output, 1
    ...
    test 'should accept attributes as array' do
      sample_input_path = fixture_path 'sample.tipi'
      output = Tipi.convert_file sample_input_path, attributes: %w(sectnums idprefix idseparator=-), to_file: false
      assert_css '#section-a', output, 1
    end

    test 'should accept attributes as string' do
      sample_input_path = fixture_path 'sample.tipi'
      output = Tipi.convert_file sample_input_path, attributes: 'sectnums idprefix idseparator=-', to_file: false
      assert_css '#section-a', output, 1
    end

    test 'should link to default stylesheet by default when safe mode is SECURE or greater' do
      sample_input_path = fixture_path 'basic.tipi'
      output = Tipi.convert_file sample_input_path, standalone: true, to_file: false
      assert_css 'html:root > head > link[rel="stylesheet"][href^="https://fonts.googleapis.com"]', output, 1
      assert_css 'html:root > head > link[rel="stylesheet"][href="./tipi.css"]', output, 1
    end
    ...
    test 'should embed default stylesheet by default if SafeMode is less than SECURE' do
      ...
      output = Tipi.convert input, safe: Tipi::SafeMode::SERVER, standalone: true
      assert_css 'html:root > head > link[rel="stylesheet"][href^="https://fonts.googleapis.com"]', output, 1
      assert_css 'html:root > head > link[rel="stylesheet"][href="./tipi.css"]', output, 0
    ...
    test 'should embed remote stylesheet by default if SafeMode is less than SECURE and allow-uri-read is set' do
      ...
      output = using_test_webserver do
        Tipi.convert input, safe: Tipi::SafeMode::SERVER, standalone: true, attributes: { 'allow-uri-read' => '', 'stylesheet' => %(http://#{resolve_localhost}:9876/fixtures/custom.css) }
      end
      stylenode = xmlnodes_at_css 'html:root > head > style', output, 1
      styles = stylenode.content
      refute_nil styles
    ...
    test 'should not allow linkcss be unset from document if SafeMode is SECURE or greater' do
      ...
      output = Tipi.convert input, standalone: true
      assert_css 'html:root > head > link[rel="stylesheet"][href^="https://fonts.googleapis.com"]', output, 1
      assert_css 'html:root > head > link[rel="stylesheet"][href="./tipi.css"]', output, 1
    end
    ...
    test 'should embed default stylesheet if linkcss is unset from API and SafeMode is SECURE or greater' do
      ...
      [{ 'linkcss!' => '' }, { 'linkcss' => nil }, { 'linkcss' => false }].each do |attrs|
        output = Tipi.convert input, standalone: true, attributes: attrs
        assert_css 'html:root > head > link[rel="stylesheet"][href^="https://fonts.googleapis.com"]', output, 1
        assert_css 'html:root > head > link[rel="stylesheet"][href="./tipi.css"]', output, 0
        stylenode = xmlnodes_at_css 'html:root > head > style', output, 1
        styles = stylenode.content
    ...
    test 'should embed default stylesheet if safe mode is less than SECURE and linkcss is unset from API' do
      sample_input_path = fixture_path 'basic.tipi'
      output = Tipi.convert_file sample_input_path,
        standalone: true, to_file: false, safe: Tipi::SafeMode::SAFE, attributes: { 'linkcss!' => '' }
    ...
    test 'should not link to stylesheet if stylesheet is unset' do
      ...
      output = Tipi.convert input, standalone: true, attributes: { 'stylesheet!' => '' }
    ...
    test 'should link to custom stylesheet if specified in stylesheet attribute' do
      ...
      output = Tipi.convert input, standalone: true, attributes: { 'stylesheet' => './custom.css' }
      assert_css 'html:root > head > link[rel="stylesheet"][href^="https://fonts.googleapis.com"]', output, 0
      assert_css 'html:root > head > link[rel="stylesheet"][href="./custom.css"]', output, 1

      output = Tipi.convert input, standalone: true, attributes: { 'stylesheet' => 'file:///home/username/custom.css' }
    ...
    test 'should resolve custom stylesheet relative to stylesdir' do
      ...
      output = Tipi.convert input, standalone: true, attributes: { 'stylesheet' => 'custom.css', 'stylesdir' => './stylesheets' }
    ...
    test 'should resolve custom stylesheet to embed relative to stylesdir' do
      sample_input_path = fixture_path 'basic.tipi'
      output = Tipi.convert_file sample_input_path,
        standalone: true, safe: Tipi::SafeMode::SAFE, to_file: false, attributes: { 'stylesheet' => 'custom.css', 'stylesdir' => './stylesheets', 'linkcss!' => '' }
      stylenode = xmlnodes_at_css 'html:root > head > style', output, 1
      styles = stylenode.content
      refute_nil styles
      refute_empty styles.strip
    ...
    test 'should embed custom stylesheet read from classloader URI', if: jruby? do
      ...
      output = Tipi.convert input, safe: :unsafe, standalone: true, attributes: { 'stylesdir' => 'uri:classloader:/styles-in-jar', 'stylesheet' => 'custom.css' }
      stylenode = xmlnodes_at_css 'html:root > head > style', output, 1
      styles = stylenode.content
      refute_nil styles
      refute_empty styles.strip
    ...
    test 'should embed custom stylesheet in remote stylesdir if SafeMode is less than SECURE and allow-uri-read is set' do
      ...
      output = using_test_webserver do
        Tipi.convert input, safe: Tipi::SafeMode::SERVER, standalone: true, attributes: { 'allow-uri-read' => '', 'stylesdir' => %(http://#{resolve_localhost}:9876/fixtures), 'stylesheet' => 'custom.css' }
      end
      stylenode = xmlnodes_at_css 'html:root > head > style', output, 1
      styles = stylenode.content
      refute_nil styles
    ...
    test 'should copy custom stylesheet in folder to same folder in destination dir if copycss is set' do
      begin
        ...
        Tipi.convert_file sample_input_path,
    ...
    test 'should convert source file specified by pathname and write result to adjacent file by default' do
      ...
      begin
        doc = Tipi.convert_file sample_input_path, safe: :safe
        assert_equal sample_output_path.expand_path.to_s, (doc.attr 'outfile')
        assert sample_output_path.file?
        output = sample_output_path.read mode: Tipi::FILE_READ_MODE
        refute_empty output
        assert_xpath '/html', output, 1
        assert_xpath '/html/head', output, 1
        assert_xpath '/html/body', output, 1
    ...
    test 'should convert source file and write to specified file' do
      ...
      begin
        Tipi.convert_file sample_input_path, to_file: sample_output_path
        assert_path_exists sample_output_path
        output = File.read sample_output_path, mode: Tipi::FILE_READ_MODE
        refute_empty output
        assert_xpath '/html', output, 1
    ...
    test 'should convert source file and write to specified file in base_dir' do
      ...
      begin
        Tipi.convert_file sample_input_path, to_file: 'result.html', base_dir: fixture_dir
        assert_path_exists sample_output_path
        output = File.read sample_output_path, mode: Tipi::FILE_READ_MODE
        refute_empty output
        assert_xpath '/html', output, 1
    ...
    test 'should resolve :to_dir option correctly when both :to_dir and :to_file options are set to an absolute path' do
      begin
        ...
        doc = Tipi.convert_file sample_input_path, to_file: sample_output_path, to_dir: sample_output_dir, safe: :unsafe
        assert_path_exists sample_output_path
    ...
    test 'in_place option is ignored when to_file is specified' do
      ...
      sample_output_path = fixture_path 'result.html'
      begin
        Tipi.convert_file sample_input_path, to_file: sample_output_path, in_place: true
        assert_path_exists sample_output_path
      ensure
    ...
    test 'in_place option is ignored when to_dir is specified' do
      ...
      sample_output_path = fixture_path 'sample.html'
      begin
        Tipi.convert_file sample_input_path, to_dir: File.dirname(sample_output_path), in_place: true
        assert_path_exists sample_output_path
      ensure
        FileUtils.rm sample_output_path if File.exist? sample_output_path
      end
    ...
    test 'should set outfilesuffix to match file extension of target file' do
      ...
      begin
        Tipi.convert sample_input, to_file: sample_output_path
        assert_path_exists sample_output_path
        output = File.read sample_output_path, mode: Tipi::FILE_READ_MODE
        refute_empty output
    ...
    test 'should respect outfilesuffix soft set from API' do
      ...
      begin
        Tipi.convert_file sample_input_path, to_dir: (File.dirname sample_input_path), attributes: { 'outfilesuffix' => '.htm@' }
        assert_path_exists sample_output_path
      ensure
        FileUtils.rm sample_output_path
      end
    ...
    test 'output should be relative to to_dir option' do
      ...
      sample_output_path = File.join output_dir, 'sample.html'
      begin
        Tipi.convert_file sample_input_path, to_dir: output_dir
        assert_path_exists sample_output_path
      ensure
        FileUtils.rm sample_output_path if File.exist? sample_output_path
    ...
    test 'missing directories should be created if mkdirs is enabled' do
      ...
      sample_output_path = File.join output_dir, 'sample.html'
      begin
        Tipi.convert_file sample_input_path, to_dir: output_dir, mkdirs: true
        assert_path_exists sample_output_path
      ensure
        FileUtils.rm sample_output_path if File.exist? sample_output_path
        FileUtils.rmdir output_dir
    ...
    test 'should raise exception if an attempt is made to overwrite input file' do
      ...
      assert_raises IOError do
        Tipi.convert_file sample_input_path, attributes: { 'outfilesuffix' => '.tipi' }
      end
    end
    ...
    test 'to_file should be relative to to_dir when both given' do
      ...
      begin
        Tipi.convert_file sample_input_path, to_dir: base_dir, to_file: sample_rel_output_path
        assert_path_exists sample_output_path
      ensure
        FileUtils.rm sample_output_path if File.exist? sample_output_path
        FileUtils.rmdir output_dir
    ...
    test 'should not modify options argument' do
      ...
      options.freeze
      sample_input_path = fixture_path 'sample.tipi'
      doc = Tipi.convert_file sample_input_path, options
      refute_same options, doc.options
    end
    ...
    test 'should set to_dir option to parent directory of specified output file' do
      ...
      begin
        doc = Tipi.convert_file sample_input_path, to_file: sample_output_path
    ...
    test 'should set to_dir option to parent directory of specified output directory and file' do
      ...
      begin
        doc = Tipi.convert_file sample_input_path, to_dir: fixture_parent_path, to_file: sample_output_relpath
    ...
    test 'timings are recorded for each step' do
      sample_input_path = fixture_path 'tipi_index.txt'
      Tipi.convert_file sample_input_path, timings: (timings = Tipi::Timings.new), to_file: false
      refute_equal '0.00000', (sprintf '%05.5f', timings.read_parse)
      refute_equal '0.00000', (sprintf '%05.5f', timings.convert)
      refute_equal timings.read_parse, timings.total
    end
    ...
    test 'can override syntax highlighter using syntax_highlighters option' do
      syntax_hl = Class.new Tipi::SyntaxHighlighter::Base do
        def highlight?
          true
        end
      ...
      ----
      EOS
      output = Tipi.convert input, safe: :safe, syntax_highlighters: { 'coderay' => syntax_hl }, attributes: { 'source-highlighter' => 'coderay' }
  ...
  context 'AST' do
    ...
    test 'should not crash if nil cell text is passed to Cell constructor' do
      ...
      cell = Tipi::Table::Cell.new table.rows.body[0][0].column, nil
      refute cell.style
      assert_same Tipi::AbstractNode::NORMAL_SUBS, cell.subs
    ...
    test 'table cell should be a block' do
      ...
      assert_kind_of Tipi::AbstractBlock, cell
      assert cell.block?
      refute cell.inline?
    end
  ...
  context 'SafeMode' do
    test 'should return nil if safe mode value not recognized' do
      assert_nil Tipi::SafeMode.name_for_value 99
    end

    test 'should return nil if safe mode name not recognized' do
      assert_nil Tipi::SafeMode.value_for_name 'unknown'
    end
  end
end
