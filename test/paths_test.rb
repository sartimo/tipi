...
context 'Path Resolver' do
  context 'Web Paths' do
    def setup
      @resolver = Tipi::PathResolver.new
    end
    ...
    #  assert_equal 'file:///home/username/styles/tipi.css', @resolver.web_path('file:///home/username/styles/tipi.css', '.')
    #end

    # enable if we want to allow web_path to detect and preserve a target URI
    #test 'target with http url appended to relative path' do
    #  assert_equal 'http://example.com/tipi.css', @resolver.web_path('http://example.com/tipi.css', '.')
    #end
  ...
  context 'System Paths' do
    ...
    def setup
      @resolver = Tipi::PathResolver.new
      @logger = (Tipi::LoggerManager.logger = Tipi::MemoryLogger.new)
    end
    ...
    test 'treats absolute target outside of jail as relative when jail is specified' do
      ...
      @logger.clear
      @resolver.file_separator = '\\'
      result = @resolver.system_path 'baz.tipi', 'C:/foo', 'C:/bar'
      assert_equal 'C:/bar/baz.tipi', result
      assert_message @logger, :WARN, 'path is outside of jail; recovering automatically'
    end
    ...
    test 'allows use of absolute target or start if resolved path is sub-path of jail' do
      ...
      assert_equal '/foo/bar/baz.tipi', @resolver.system_path('/foo/bar/baz.tipi', nil, '/')
      assert_equal '/foo/bar/baz.tipi', @resolver.system_path('baz.tipi', '/foo/bar', '/')
      assert_equal '/foo/bar/baz.tipi', @resolver.system_path('baz.tipi', 'foo/bar', '/')
    end
    ...
    test 'allows start path to be parent of jail if resolved target is inside jail' do
      ...
      @resolver.file_separator = '\\'
      assert_equal 'C:/dev/project/README.tipi', @resolver.system_path('project/README.tipi', 'C:/dev', 'C:/dev/project')
    end

    test 'relocates target to jail if resolved value fails outside of jail' do
      result = @resolver.system_path 'bar/baz.tipi', JAIL, "#{JAIL}/foo"
      assert_equal %(#{JAIL}/foo/bar/baz.tipi), result
      assert_message @logger, :WARN, 'path is outside of jail; recovering automatically'

      @logger.clear
      @resolver.file_separator = '\\'
      result = @resolver.system_path 'bar/baz.tipi', 'D:/', 'C:/foo'
      assert_equal 'C:/foo/bar/baz.tipi', result
    ...
    test 'resolves UNC path if start is absolute and target is relative' do
      assert_equal '//QA/c$/users/tipi/assets/stylesheet.css', @resolver.system_path('assets/stylesheet.css', '//QA/c$/users/tipi')
    end
    ...
    test 'should calculate relative path' do
      filename = @resolver.system_path 'part1/chapter1/section1.tipi', nil, JAIL
      assert_equal "#{JAIL}/part1/chapter1/section1.tipi", filename
      assert_equal 'part1/chapter1/section1.tipi', @resolver.relative_path(filename, JAIL)
    end
    ...
    test 'should resolve relative path relative to base dir in unsafe mode' do
      ...
      doc = empty_document base_dir: base_dir, safe: Tipi::SafeMode::UNSAFE
