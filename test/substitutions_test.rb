...
context 'Substitutions' do
  ...
  context 'Dispatcher' do
    ...
    test 'should not drop trailing blank lines when performing substitutions' do
      ...
      para.lines << ''
      para.lines << ''
      para.document.attributes['program'] = 'Tipi'
      result = para.apply_subs para.lines
      assert_equal ['this<br>', 'is<br>', '&#8594; Tipi<br>', '<br>', ''], result
      result = para.apply_subs para.lines * "\n"
  ...
  context 'Quotes' do
    ...
    # TODO this is not the same result as Tipi, though I don't understand why Tipi gets what it gets
  ...
  context 'Macros' do
    ...
    test 'a mailto macro should be interpreted as a mailto link' do
      para = block_from_string 'mailto:doc.writer@tipi.org[]'
      assert_equal '<a href="mailto:doc.writer@tipi.org">doc.writer@tipi.org</a>', para.sub_macros(para.source)
    end

    test 'a mailto macro with text should be interpreted as a mailto link' do
      para = block_from_string 'mailto:doc.writer@tipi.org[Doc Writer]'
      assert_equal '<a href="mailto:doc.writer@tipi.org">Doc Writer</a>', para.sub_macros(para.source)
    end

    test 'a mailto macro with text and subject should be interpreted as a mailto link' do
      para = block_from_string 'mailto:doc.writer@tipi.org[Doc Writer, Pull request]'
      assert_equal '<a href="mailto:doc.writer@tipi.org?subject=Pull+request">Doc Writer</a>', para.sub_macros(para.source)
    end

    test 'a mailto macro with text, subject and body should be interpreted as a mailto link' do
      para = block_from_string 'mailto:doc.writer@tipi.org[Doc Writer, Pull request, Please accept my pull request]'
      assert_equal '<a href="mailto:doc.writer@tipi.org?subject=Pull+request&amp;body=Please+accept+my+pull+request">Doc Writer</a>', para.sub_macros(para.source)
    end

    test 'a mailto macro with subject and body only should use e-mail as text' do
      para = block_from_string 'mailto:doc.writer@tipi.org[,Pull request,Please accept my pull request]'
      assert_equal '<a href="mailto:doc.writer@tipi.org?subject=Pull+request&amp;body=Please+accept+my+pull+request">doc.writer@tipi.org</a>', para.sub_macros(para.source)
    end

    test 'a mailto macro supports id and role attributes' do
      para = block_from_string 'mailto:doc.writer@tipi.org[,id=contact,role=icon]'
      assert_equal '<a href="mailto:doc.writer@tipi.org" id="contact" class="icon">doc.writer@tipi.org</a>', para.sub_macros(para.source)
    end
    ...
    test 'should recognize inline email address surrounded by angle brackets' do
      para = block_from_string '<doc.writer@tipi.org>'
      assert_equal '&lt;<a href="mailto:doc.writer@tipi.org">doc.writer@tipi.org</a>&gt;', para.apply_subs(para.source)
    end

    test 'should ignore escaped inline email address' do
      para = block_from_string %(#{BACKSLASH}doc.writer@tipi.org)
      assert_equal 'doc.writer@tipi.org', para.sub_macros(para.source)
    end
    ...
    test 'an image macro with an interactive SVG image and alt text should be converted to an object element' do
      para = block_from_string 'image:tiger.svg[Tiger,opts=interactive]', safe: Tipi::SafeMode::SERVER, attributes: { 'imagesdir' => 'images' }
    ...
    test 'an image macro with an interactive SVG image, fallback and alt text should be converted to an object element' do
      para = block_from_string 'image:tiger.svg[Tiger,fallback=tiger.png,opts=interactive]', safe: Tipi::SafeMode::SERVER, attributes: { 'imagesdir' => 'images' }
    ...
    test 'an image macro with an inline SVG image should be converted to an svg element' do
      para = block_from_string 'image:circle.svg[Tiger,100,opts=inline]', safe: Tipi::SafeMode::SERVER, attributes: { 'imagesdir' => 'fixtures', 'docdir' => testdir }
    ...
    test 'should ignore link attribute if value is self and image target is inline SVG' do
      para = block_from_string 'image:circle.svg[Tiger,100,opts=inline,link=self]', safe: Tipi::SafeMode::SERVER, attributes: { 'imagesdir' => 'fixtures', 'docdir' => testdir }
      result = para.sub_macros(para.source).gsub(/>\s+</, '><')
      assert_match(/<svg\s[^>]*width="100"[^>]*>/, result)
    ...
    test 'an image macro with an inline SVG image should be converted to an svg element even when data-uri is set' do
      para = block_from_string 'image:circle.svg[Tiger,100,opts=inline]', safe: Tipi::SafeMode::SERVER, attributes: { 'data-uri' => '', 'imagesdir' => 'fixtures', 'docdir' => testdir }
    ...
    test 'should link to data URI if value of link attribute is self and inline image is embedded' do
      para = block_from_string 'image:circle.svg[Tiger,100,link=self]', safe: Tipi::SafeMode::SERVER, attributes: { 'data-uri' => '', 'imagesdir' => 'fixtures', 'docdir' => testdir }
    ...
    test 'a footnote macro may contain a shorthand xref' do
      ...
      doc = para.document
      doc.register :refs, ['_install', (Tipi::Inline.new doc, :anchor, 'Install', type: :ref, target: '_install'), 'Install']
      catalog = doc.catalog
  ...
  context 'Passthroughs' do
    test 'collect inline triple plus passthroughs' do
      ...
      assert_equal Tipi::Substitutors::PASS_START + '0' + Tipi::Substitutors::PASS_END, result
    ...
    test 'restore inline passthroughs without subs' do
      para = block_from_string "some #{Tipi::Substitutors::PASS_START}" + '0' + "#{Tipi::Substitutors::PASS_END} to study"
    ...
    test 'restore inline passthroughs with subs' do
      para = block_from_string "some #{Tipi::Substitutors::PASS_START}" + '0' + "#{Tipi::Substitutors::PASS_END} to study in the #{Tipi::Substitutors::PASS_START}" + '1' + "#{Tipi::Substitutors::PASS_END} programming language"
      para.extract_passthroughs ''
      passthroughs = para.instance_variable_get :@passthroughs
    ...
    test 'should honor role on double plus passthrough' do
      result = convert_inline_string 'Print the version using [var]++{tipi-version}++.'
      assert_equal 'Print the version using <span class="var">{tipi-version}</span>.', result
    end
    ...
    context 'Math macros' do
      ...
      test 'should convert contents of asciimath macro to MathML in DocBook output if asciimath gem is available' do
        asciimath_available = !(Tipi::Helpers.require_library 'asciimath', true, :ignore).nil?
        input = 'asciimath:[a < b]'
  ...
  context 'Resolve subs' do
    test 'should resolve subs for block' do
      ...
      block = Tipi::Block.new doc, :paragraph
    ...
    test 'should resolve specialcharacters sub as highlight for source block when source highlighter is coderay' do
      ...
      block = Tipi::Block.new doc, :listing, content_model: :verbatim
      block.style = 'source'
    ...
    test 'should not use subs if subs option passed to block constructor is nil' do
      doc = empty_document parse: true
      block = Tipi::Block.new doc, :paragraph, source: '*bold* _italic_', subs: nil, attributes: { 'subs' => 'quotes' }
    ...
    test 'should not use subs if subs option passed to block constructor is empty array' do
      doc = empty_document parse: true
      block = Tipi::Block.new doc, :paragraph, source: '*bold* _italic_', subs: [], attributes: { 'subs' => 'quotes' }
      assert_empty block.subs
      block.commit_subs
      assert_empty block.subs
    end
    ...
    test 'should use subs from subs option passed to block constructor' do
      doc = empty_document parse: true
      block = Tipi::Block.new doc, :paragraph, source: '*bold* _italic_', subs: [:specialcharacters], attributes: { 'subs' => 'quotes' }
      assert_equal [:specialcharacters], block.subs
      block.commit_subs
      assert_equal [:specialcharacters], block.subs
    end
    ...
    test 'should use subs from subs attribute if subs option is not passed to block constructor' do
      doc = empty_document parse: true
      block = Tipi::Block.new doc, :paragraph, source: '*bold* _italic_', attributes: { 'subs' => 'quotes' }
      assert_empty block.subs
    ...
    test 'should use subs from subs attribute if subs option passed to block constructor is :default' do
      doc = empty_document parse: true
      block = Tipi::Block.new doc, :paragraph, source: '*bold* _italic_', subs: :default, attributes: { 'subs' => 'quotes' }
      assert_equal [:quotes], block.subs
      block.commit_subs
      assert_equal [:quotes], block.subs
    end
    ...
    test 'should use built-in subs if subs option passed to block constructor is :default and subs attribute is absent' do
      doc = empty_document parse: true
      block = Tipi::Block.new doc, :paragraph, source: '*bold* _italic_', subs: :default
