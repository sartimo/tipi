...
context 'Attributes' do
  ...
  context 'Assignment' do
    ...
    # NOTE Tipi.py recognizes this entry
    ...
    # NOTE Tipi.py does not recognize this entry
    ...
    test 'creates an attribute by fusing a legacy multi-line value' do
      ...
      assert_equal 'This is the first Ruby implementation of Tipi.', doc.attributes['description']
    end
    ...
    test 'resolves user-home attribute if safe mode is less than SERVER' do
      ...
      assert_equal %(#{Tipi::USER_HOME}/etc/images), output
    end
    ...
    test 'backend attributes are updated if backend attribute is defined in document and safe mode is less than SERVER' do
      ...
      doc = document_from_string input, safe: Tipi::SafeMode::SAFE
      expect = {
        'backend' => 'docbook5',
        'backend-docbook5' => '',
        'backend-docbook5-doctype-book' => '',
    ...
    test 'backend attributes defined in document options overrides backend attribute in document' do
      doc = document_from_string ':backend: docbook5', safe: Tipi::SafeMode::SAFE, attributes: { 'backend' => 'html5' }
    ...
    test 'can only access a positional attribute from the attributes hash' do
      node = Tipi::Block.new nil, :paragraph, attributes: { 1 => 'position 1' }
    ...
    test 'set_attr should set value to empty string if no value is specified' do
      node = Tipi::Block.new nil, :paragraph, attributes: {}
      node.set_attr 'foo'
    ...
    test 'remove_attr should remove attribute and return previous value' do
      doc = empty_document
      node = Tipi::Block.new doc, :paragraph, attributes: { 'foo' => 'bar' }
    ...
    test 'set_attr should not overwrite existing key if overwrite is false' do
      node = Tipi::Block.new nil, :paragraph, attributes: { 'foo' => 'bar' }
    ...
    test 'set_attr should set header attribute in loaded document' do
      ...
      doc = Tipi.load input, attributes: { 'uri' => 'https://github.com' }
  ...
  context 'Interpolation' do
    ...
    test 'substitutes inside block title' do
      ...
      assert_xpath '//*[@class="title"]/code[text()="tipi"]', output, 1
    ...
    test 'does not show docdir and shows relative docfile if safe mode is SERVER or greater' do
      ...
      * docfile: {docfile}
      ...
      docdir = Dir.pwd
      docfile = File.join docdir, 'sample.tipi'
      output = convert_string_to_embedded input, safe: Tipi::SafeMode::SERVER, attributes: { 'docdir' => docdir, 'docfile' => docfile }
      assert_xpath '//li[1]/p[text()="docdir: "]', output, 1
      assert_xpath '//li[2]/p[text()="docfile: sample.tipi"]', output, 1
    end
    ...
    test 'shows absolute docdir and docfile paths if safe mode is less than SERVER' do
      ...
      * docfile: {docfile}
      ...
      docdir = Dir.pwd
      docfile = File.join docdir, 'sample.tipi'
      output = convert_string_to_embedded input, safe: Tipi::SafeMode::SAFE, attributes: { 'docdir' => docdir, 'docfile' => docfile }
  ...
  context 'Intrinsic attributes' do
    test 'substitute intrinsics' do
      Tipi::INTRINSIC_ATTRIBUTES.each_pair do |key, value|
