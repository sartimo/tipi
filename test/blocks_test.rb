...
context 'Blocks' do
  default_logger = Tipi::LoggerManager.logger

  setup do
    Tipi::LoggerManager.logger = (@logger = Tipi::MemoryLogger.new)
  end

  teardown do
    Tipi::LoggerManager.logger = default_logger
  end
  ...
  context 'Layout Breaks' do
    ...
    test 'horizontal rule with markdown syntax disabled' do
      old_markdown_syntax = Tipi::Compliance.markdown_syntax
      begin
        Tipi::Compliance.markdown_syntax = false
        %w(''' '''' '''''').each do |line|
          output = convert_string_to_embedded line
          assert_includes output, '<hr>'
        end
        ...
        Tipi::Compliance.markdown_syntax = old_markdown_syntax
      end
    end
  ...
  context 'Quote and Verse Blocks' do
    ...
    test 'verse should have normal subs' do
      ...
      assert_equal Tipi::Substitutors::NORMAL_SUBS, verse.subs
    end
  ...
  context 'Example Blocks' do
    ...
    test 'assigns sequential numbered caption to example block with title' do
      input = <<~'EOS'
      .Writing Docs with Tipi
      ====
      Here's how you write Tipi.
      ...
      output = doc.convert
      assert_xpath '(//*[@class="exampleblock"])[1]/*[@class="title"][text()="Example 1. Writing Docs with Tipi"]', output, 1
    ...
    test 'assigns sequential character caption to example block with title' do
      ...
      output = doc.convert
      assert_xpath '(//*[@class="exampleblock"])[1]/*[@class="title"][text()="Example A. Writing Docs with Tipi"]', output, 1
    ...
    test 'should use explicit caption if specified' do
      ...
      output = doc.convert
      assert_xpath '(//*[@class="exampleblock"])[1]/*[@class="title"][text()="Look! Writing Docs with Tipi"]', output, 1
  ...
  context 'Passthrough Blocks' do
    ...
    test 'passthrough block honors explicit subs list' do
      ...
      expected = %(This is a <em>passthrough</em> block.\n<a href="http://tipi.org" class="bare">http://tipi.org</a>)
      output = convert_string_to_embedded input
      assert_equal expected, output.strip
    end
  ...
  context 'Images' do
    ...
    test 'converts SVG image using img element by default' do
      ...
      output = convert_string_to_embedded input, safe: Tipi::SafeMode::SERVER
    ...
    test 'converts to inline SVG image when inline option is set on block' do
      ...
      output = convert_string_to_embedded input, safe: Tipi::SafeMode::SERVER, attributes: { 'docdir' => testdir }
    ...
    test 'should not crash if explicit width on SVG image block is an integer' do
      ...
      doc = document_from_string input, safe: Tipi::SafeMode::SERVER, attributes: { 'docdir' => testdir }
    ...
    test 'can convert block image with caption' do
      ...
      output = doc.convert
      assert_xpath '//*[@class="imageblock"]//img[@src="images/tiger.png"][@alt="Tiger"]', output, 1
      assert_xpath '//*[@class="imageblock"]/*[@class="title"][text()="Figure 1. The Tipi Tiger"]', output, 1
    ...
    test 'can convert block image with explicit caption' do
      ...
      output = doc.convert
      assert_xpath '//*[@class="imageblock"]//img[@src="images/tiger.png"][@alt="Tiger"]', output, 1
      assert_xpath '//*[@class="imageblock"]/*[@class="title"][text()="Voila! The Tipi Tiger"]', output, 1
    ...
    test 'should pass through image that references uri' do
      ...
      image::http://tipi.org/images/tiger.png[Tiger]
      EOS

      output = convert_string_to_embedded input
      assert_xpath '/*[@class="imageblock"]//img[@src="http://tipi.org/images/tiger.png"][@alt="Tiger"]', output, 1
    end
    ...
    test 'embeds base64-encoded data uri for image when data-uri attribute is set' do
      ...
      doc = document_from_string input, safe: Tipi::SafeMode::SAFE, attributes: { 'docdir' => testdir }
    ...
    test 'embeds base64-encoded data uri for image in classloader when data-uri attribute is set', if: jruby? do
      ...
      doc = document_from_string input, safe: Tipi::SafeMode::UNSAFE, attributes: { 'docdir' => testdir }
  ...
  context 'Admonition icons' do
    test 'can resolve icon relative to default iconsdir' do
      ...
      output = convert_string input, safe: Tipi::SafeMode::SERVER
    ...
    test 'should import Font Awesome and use font-based icons when value of icons attribute is font' do
      ...
      output = convert_string input, safe: Tipi::SafeMode::SERVER
      assert_css %(html > head > link[rel="stylesheet"][href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/#{Tipi::FONT_AWESOME_VERSION}/css/font-awesome.min.css"]), output, 1
    ...
    test 'should use http uri scheme for assets when asset-uri-scheme is http' do
      ...
      assert_css %(html > head > link[rel="stylesheet"][href="http://cdnjs.cloudflare.com/ajax/libs/font-awesome/#{Tipi::FONT_AWESOME_VERSION}/css/font-awesome.min.css"]), output, 1
      assert_css %(html > body > script[src="http://cdnjs.cloudflare.com/ajax/libs/highlight.js/#{Tipi::HIGHLIGHT_JS_VERSION}/highlight.min.js"]), output, 1
    end
    ...
    test 'should use no uri scheme for assets when asset-uri-scheme is blank' do
      ...
      output = convert_string input, safe: Tipi::SafeMode::SAFE
      assert_css %(html > head > link[rel="stylesheet"][href="//cdnjs.cloudflare.com/ajax/libs/font-awesome/#{Tipi::FONT_AWESOME_VERSION}/css/font-awesome.min.css"]), output, 1
      assert_css %(html > body > script[src="//cdnjs.cloudflare.com/ajax/libs/highlight.js/#{Tipi::HIGHLIGHT_JS_VERSION}/highlight.min.js"]), output, 1
    end
  end

  context 'Image paths' do
    test 'restricts access to ancestor directories when safe mode level is at least SAFE' do
      input = 'image::tipi.png[Tipi]'
      basedir = testdir
      block = block_from_string input, attributes: { 'docdir' => basedir }
      doc = block.document
      assert doc.safe >= Tipi::SafeMode::SAFE
    ...
    test 'does not restrict access to ancestor directories when safe mode is disabled' do
      input = 'image::tipi.png[Tipi]'
      basedir = testdir
      block = block_from_string input, safe: Tipi::SafeMode::UNSAFE, attributes: { 'docdir' => basedir }
      doc = block.document
      assert_equal Tipi::SafeMode::UNSAFE, doc.safe
  ...
  context 'Substitutions' do
    ...
    test 'should be able to prepend, append and remove subs from default block substitution list' do
      ...
      result = doc.convert
      assert_includes result, '<pre><a href="https://tipi.org">&gt;&gt;</a> &lt;1&gt;</pre>'
    end
