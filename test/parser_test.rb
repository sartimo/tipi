...
context 'Parser' do
  test 'is_section_title?' do
    assert Tipi::Parser.is_section_title?('Tipi Home Page', '==================')
    assert Tipi::Parser.is_section_title?('=== Tipi Home Page')
  end

  test 'sanitize attribute name' do
    assert_equal 'foobar', Tipi::Parser.sanitize_attribute_name('Foo Bar')
    assert_equal 'foo', Tipi::Parser.sanitize_attribute_name('foo')
    assert_equal 'foo3-bar', Tipi::Parser.sanitize_attribute_name('Foo 3^ # - Bar[')
  end

  test 'store attribute with value' do
    attr_name, attr_value = Tipi::Parser.store_attribute 'foo', 'bar'
  ...
  test 'store attribute with negated value' do
    { 'foo!' => nil, '!foo' => nil, 'foo' => nil }.each do |name, value|
      attr_name, attr_value = Tipi::Parser.store_attribute name, value
  ...
  test 'store accessible attribute on document with value' do
    ...
    attrs = {}
    attr_name, attr_value = Tipi::Parser.store_attribute 'foo', 'bar', doc, attrs
    assert_equal 'foo', attr_name
    assert_equal 'bar', attr_value
  ...
  test 'store accessible attribute on document with value that contains attribute reference' do
    ...
    attrs = {}
    attr_name, attr_value = Tipi::Parser.store_attribute 'foo', '{release}', doc, attrs
    assert_equal 'foo', attr_name
  ...
  test 'store accessible attribute on document with negated value' do
    { 'foo!' => nil, '!foo' => nil, 'foo' => nil }.each do |name, value|
      ...
      doc.set_attribute 'foo', 'baz'
      attrs = {}
      attr_name, attr_value = Tipi::Parser.store_attribute name, value, doc, attrs
      assert_equal name.sub('!', ''), attr_name
      assert_nil attr_value
      assert attrs.key?(:attribute_entries)
      assert_equal 1, attrs[:attribute_entries].size
  ...
  test 'parse style attribute with id and role' do
    ...
    style = Tipi::Parser.parse_style_attribute attributes
  ...
  test 'parse author first email' do
    metadata = parse_header_metadata 'Stuart <founder@tipi.org>'
    assert_equal 6, metadata.size
    assert_equal 1, metadata['authorcount']
    assert_equal 'Stuart', metadata['author']
    assert_equal metadata['author'], metadata['authors']
    assert_equal 'Stuart', metadata['firstname']
    assert_equal 'founder@tipi.org', metadata['email']
    assert_equal 'S', metadata['authorinitials']
  end

  test 'parse author first last email' do
    metadata = parse_header_metadata 'Stuart Rackham <founder@tipi.org>'
    assert_equal 7, metadata.size
    assert_equal 1, metadata['authorcount']
  ...
  test 'parse invalid author line becomes author' do
    metadata = parse_header_metadata '   Stuart       Rackham, founder of Tipi   <founder@tipi.org>'
    assert_equal 5, metadata.size
    assert_equal 1, metadata['authorcount']
    assert_equal 'Stuart Rackham, founder of Tipi <founder@tipi.org>', metadata['author']
    assert_equal metadata['author'], metadata['authors']
    assert_equal 'Stuart Rackham, founder of Tipi <founder@tipi.org>', metadata['firstname']
    assert_equal 'S', metadata['authorinitials']
  end

  test 'parse multiple authors' do
    metadata = parse_header_metadata 'Doc Writer <doc.writer@tipi.org>; John Smith <john.smith@tipi.org>'
  ...
  test 'skips blank author entries in implicit author line' do
    metadata = parse_header_metadata 'Doc Writer; ; John Smith <john.smith@tipi.org>;'
    assert_equal 2, metadata['authorcount']
    assert_equal 'Doc Writer', metadata['author_1']
    assert_equal 'John Smith', metadata['author_2']
  end
  ...
  # Tipi recognizes a standalone revision without a trailing comma
  ...
  # while compliant w/ Tipi, this is just sloppy parsing
  ...
  test 'adjust indentation to 0' do
    ...
    lines = input.split ?\n
    Tipi::Parser.adjust_indentation! lines
  ...
  test 'adjust indentation mixed with tabs and spaces to 0' do
    ...
    lines = input.split ?\n
    Tipi::Parser.adjust_indentation! lines, 0, 4
    assert_equal expected, (lines * ?\n)
  end
  ...
  test 'adjust indentation to non-zero' do
    ...
    lines = input.split ?\n
    Tipi::Parser.adjust_indentation! lines, 2
    assert_equal expected, (lines * ?\n)
  end
  ...
  test 'preserve block indent if indent is -1' do
    ...
    lines = input.lines
    Tipi::Parser.adjust_indentation! lines, -1
