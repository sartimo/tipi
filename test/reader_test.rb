  ...
  context 'Reader' do
    context 'Prepare lines' do
      test 'should prepare lines from Array data' do
        reader = Tipi::Reader.new SAMPLE_DATA
        assert_equal SAMPLE_DATA, reader.lines
      end

      test 'should prepare lines from String data' do
        reader = Tipi::Reader.new SAMPLE_DATA.join(Tipi::LF)
        assert_equal SAMPLE_DATA, reader.lines
      end

      test 'should prepare lines from String data with trailing newline' do
        reader = Tipi::Reader.new SAMPLE_DATA.join(Tipi::LF) + Tipi::LF
        assert_equal SAMPLE_DATA, reader.lines
      end

      test 'should remove UTF-8 BOM from first line of String data' do
        %w(UTF-8 ASCII-8BIT).each do |start_encoding|
          data = String.new %(\xef\xbb\xbf#{SAMPLE_DATA.join Tipi::LF}), encoding: start_encoding
          reader = Tipi::Reader.new data, nil, normalize: true
      ...
      test 'should encode UTF-16LE string to UTF-8 when BOM is found' do
        %w(UTF-8 ASCII-8BIT).each do |start_encoding|
          data = "\ufeff#{SAMPLE_DATA.join Tipi::LF}".encode('UTF-16LE').force_encoding start_encoding
          reader = Tipi::Reader.new data, nil, normalize: true
          assert_equal Encoding::UTF_8, reader.lines[0].encoding
          assert_equal 'f', reader.lines[0].chr
          assert_equal SAMPLE_DATA, reader.lines
      ...
      test 'should encode UTF-16BE string to UTF-8 when BOM is found' do
        %w(UTF-8 ASCII-8BIT).each do |start_encoding|
          data = "\ufeff#{SAMPLE_DATA.join Tipi::LF}".encode('UTF-16BE').force_encoding start_encoding
          reader = Tipi::Reader.new data, nil, normalize: true
          assert_equal Encoding::UTF_8, reader.lines[0].encoding
          assert_equal 'f', reader.lines[0].chr
          assert_equal SAMPLE_DATA, reader.lines
    ...
    context 'With empty data' do
      test 'has_more_lines? should return false with empty data' do
        refute Tipi::Reader.new.has_more_lines?
      end

      test 'empty? should return true with empty data' do
        assert_empty Tipi::Reader.new
        assert Tipi::Reader.new.eof?
      end

      test 'next_line_empty? should return true with empty data' do
        assert Tipi::Reader.new.next_line_empty?
      end

      test 'peek_line should return nil with empty data' do
        assert_nil Tipi::Reader.new.peek_line
      end

      test 'peek_lines should return empty Array with empty data' do
        assert_empty Tipi::Reader.new.peek_lines(1)
      end

      test 'read_line should return nil with empty data' do
        assert_nil Tipi::Reader.new.read_line
        #assert_nil Tipi::Reader.new.get_line
      end

      test 'read_lines should return empty Array with empty data' do
        assert_empty Tipi::Reader.new.read_lines
        #assert_equal [], Tipi::Reader.new.get_lines
      end
    end
    ...
    context 'With data' do
      ...
      test 'read should return all lines joined as String' do
        reader = Tipi::Reader.new SAMPLE_DATA
        assert_equal SAMPLE_DATA.join(Tipi::LF), reader.read
      end
      ...
      test 'unshift puts line onto Reader as next line to read' do
        reader = Tipi::Reader.new SAMPLE_DATA, nil, normalize: true
      ...
      test 'skip_blank_lines should skip blank lines' do
        reader = Tipi::Reader.new ['', ''].concat(SAMPLE_DATA)
    ...
    context 'Read lines until' do
      test 'Read lines until until end' do
        ...
        reader = Tipi::Reader.new lines, nil, normalize: true
      ...
      test 'Read lines until until blank line preserving last line' do
        lines = <<~'EOS'.split Tipi::LF
        This is one paragraph.

        This is another paragraph.
        EOS

        reader = Tipi::Reader.new lines
      ...
      test 'read lines until terminator' do
        ...
        ****
        ...
        reader = Tipi::PreprocessorReader.new doc, lines, nil, normalize: true
  ...
  context 'PreprocessorReader' do
    context 'Type hierarchy' do
      test 'PreprocessorReader should extend from Reader' do
        ...
        assert_kind_of Tipi::PreprocessorReader, reader
      end

      test 'PreprocessorReader should invoke or emulate Reader initializer' do
        doc = Tipi::Document.new SAMPLE_DATA
        reader = doc.reader
        assert_equal SAMPLE_DATA, reader.lines
        assert_equal 1, reader.lineno
      end
    ...
    context 'Prepare lines' do
      ...
      test 'should prepare and normalize lines from String data' do
        ...
        data.unshift ' '
        data.push ' '
        data_as_string = data * Tipi::LF
        doc = Tipi::Document.new data_as_string
        reader = doc.reader
        assert_equal [''] + SAMPLE_DATA, reader.lines
      end
      ...
      test 'should clean CRLF from end of lines' do
        ...
        [input, input.lines, input.split(Tipi::LF), input.split(Tipi::LF).join(Tipi::LF)].each do |lines|
          doc = Tipi::Document.new lines
          reader = doc.reader
      ...
      test 'should not skip front matter if ending delimiter is not found' do
        ...
        doc = Tipi::Document.new input, attributes: { 'skip-front-matter' => '' }
        reader = doc.reader
        assert_equal '---', reader.peek_line
        refute doc.attributes.key? 'front-matter'
        assert_equal 1, reader.lineno
    ...
    context 'Include Stack' do
      ...
      test 'PreprocessorReader#push_include method should set path from file automatically if not specified' do
        ...
        reader = doc.reader
        append_lines = %w(one two three)
        reader.push_include append_lines, '/tmp/lines.tipi'
        assert_equal '/tmp/lines.tipi', reader.file
        assert_equal 'lines.tipi', reader.path
      ...
      test 'PreprocessorReader#push_include method should accept file as a URI and compute dir and path' do
        file_uri = URI.parse 'http://example.com/docs/file.tipi'
    ...
    context 'Include Directive' do
      test 'should replace include directive with link macro in default safe mode' do
        input = 'include::include-file.tipi[]'
        doc = Tipi::Document.new input
        reader = doc.reader
        assert_equal 'link:include-file.tipi[role=include]', reader.read_line
      end

      test 'should escape spaces in target when generating link from include directive' do
        input = 'include::foo bar baz.tipi[]'
        doc = Tipi::Document.new input
        reader = doc.reader
        assert_equal 'link:pass:c[foo bar baz.tipi][role=include]', reader.read_line
      end

      test 'should preserve attrlist when replacing include directive with link macro' do
        input = 'include::include-file.tipi[leveloffset=+1]'
        doc = Tipi::Document.new input
        reader = doc.reader
        assert_equal 'link:include-file.tipi[role=include,leveloffset=+1]', reader.read_line
      end

      test 'should replace include directive with link macro if safe mode allows it, but allow-uri-read is not set' do
        using_memory_logger do |logger|
          input = 'include::https://example.org/dist/info.tipi[]'
          doc = Tipi::Document.new input, safe: :safe
          reader = doc.reader
          assert_equal 'link:https://example.org/dist/info.tipi[role=include]', reader.read_line
          assert_message logger, :WARN, '<stdin>: line 1: cannot include contents of URI: https://example.org/dist/info.tipi (allow-uri-read attribute not enabled)', Hash
        end
      end

      test 'should escape spaces in target when generating link from remote include directive' do
        using_memory_logger do |logger|
          input = 'include::https://example.org/no such file.tipi[]'
          doc = Tipi::Document.new input, safe: :safe
          reader = doc.reader
          assert_equal 'link:pass:c[https://example.org/no such file.tipi][role=include]', reader.read_line
          assert_message logger, :WARN, '<stdin>: line 1: cannot include contents of URI: https://example.org/no such file.tipi (allow-uri-read attribute not enabled)', Hash
        end
      end

      test 'should preserve attrlist when replacing remove include directive with link macro' do
        using_memory_logger do |logger|
          input = 'include::https://example.org/dist/info.tipi[leveloffset=+1]'
          doc = Tipi::Document.new input, safe: :safe
          reader = doc.reader
          assert_equal 'link:https://example.org/dist/info.tipi[role=include,leveloffset=+1]', reader.read_line
          assert_message logger, :WARN, '<stdin>: line 1: cannot include contents of URI: https://example.org/dist/info.tipi (allow-uri-read attribute not enabled)', Hash
        end
      end
      ...
      test 'include directive with remote target is converted to a link when allow-uri-read is not set' do
        using_memory_logger do |logger|
          input = 'include::http://example.org/team.tipi[]'
          doc = Tipi::Document.new input, safe: :safe
          reader = doc.reader
          assert_equal 'link:http://example.org/team.tipi[role=include]', reader.read_line
          assert_message logger, :WARN, '<stdin>: line 1: cannot include contents of URI: http://example.org/team.tipi (allow-uri-read attribute not enabled)', Hash
        end
      end
      ...
      test 'include directive with remote target is converted to a link when safe mode is secure' do
        using_memory_logger do |logger|
          input = 'include::http://example.org/team.tipi[]'
          doc = Tipi::Document.new input, safe: :secure
          reader = doc.reader
          assert_equal 'link:http://example.org/team.tipi[role=include]', reader.read_line
      ...
      test 'include directive is enabled when safe mode is less than SECURE' do
        input = 'include::fixtures/include-file.tipi[]'
      ...
      test 'should strip BOM from include file' do
        input = %(:showtitle:\ninclude::fixtures/file-with-utf8-bom.tipi[])
      ...
      test 'should include content from a file on the classloader', if: jruby? do
        ...
        input = 'include::uri:classloader:/includes-in-jar/include-file.tipi[]'
      ...
      test 'should not track include in catalog for non-Tipi include files' do
        input = <<~'EOS'
        ----
      ...
      test 'include directive should resolve file with spaces in name' do
        input = 'include::fixtures/include file.tipi[]'
        include_file = File.join DIRNAME, 'fixtures', 'include-file.tipi'
        include_file_with_sp = File.join DIRNAME, 'fixtures', 'include file.tipi'
        begin
      ...
      test 'include directive should resolve file with {sp} in name' do
        input = 'include::fixtures/include{sp}file.tipi[]'
        include_file = File.join DIRNAME, 'fixtures', 'include-file.tipi'
        include_file_with_sp = File.join DIRNAME, 'fixtures', 'include file.tipi'
        begin
          FileUtils.cp include_file, include_file_with_sp
      ...
      test 'include directive should not attempt to resolve target as remote if allow-uri-read is set and URL is not on first line' do
        using_memory_logger do |logger|
          input = <<~'EOS'
          :target: not-a-file.tipi + \
          http://example.org/team.tipi

          include::{target}[]
          EOS
          doc = Tipi.load input, safe: :safe, base_dir: fixturedir
          lines = doc.blocks[0].lines
          assert_equal [%(Unresolved directive in <stdin> - include::not-a-file.tipi +\nhttp://example.org/team.tipi[])], lines
          assert_message logger, :ERROR, %(<stdin>: line 4: include file not found: #{fixture_path 'not-a-file.tipi'} +\nhttp://example.org/team.tipi), Hash
        end
      end

      test 'include directive should resolve file relative to current include' do
        input = 'include::fixtures/parent-include.tipi[]'
        pseudo_docfile = File.join DIRNAME, 'main.tipi'
        fixtures_dir = File.join DIRNAME, 'fixtures'
        parent_include_docfile = File.join fixtures_dir, 'parent-include.tipi'
        child_include_docfile = File.join fixtures_dir, 'child-include.tipi'
        grandchild_include_docfile = File.join fixtures_dir, 'grandchild-include.tipi'

        doc = empty_safe_document base_dir: DIRNAME
        reader = Tipi::PreprocessorReader.new doc, input, pseudo_docfile, normalize: true

        assert_equal pseudo_docfile, reader.file
        assert_equal DIRNAME, reader.dir
        assert_equal 'main.tipi', reader.path

        assert_equal 'first line of parent', reader.read_line

        assert_equal 'fixtures/parent-include.tipi: line 1', reader.cursor_at_prev_line.to_s
        assert_equal parent_include_docfile, reader.file
        assert_equal fixtures_dir, reader.dir
        assert_equal 'fixtures/parent-include.tipi', reader.path

        reader.skip_blank_lines

        assert_equal 'first line of child', reader.read_line

        assert_equal 'fixtures/child-include.tipi: line 1', reader.cursor_at_prev_line.to_s
        assert_equal child_include_docfile, reader.file
        assert_equal fixtures_dir, reader.dir
        assert_equal 'fixtures/child-include.tipi', reader.path

        reader.skip_blank_lines

        assert_equal 'first line of grandchild', reader.read_line

        assert_equal 'fixtures/grandchild-include.tipi: line 1', reader.cursor_at_prev_line.to_s
        assert_equal grandchild_include_docfile, reader.file
        assert_equal fixtures_dir, reader.dir
        assert_equal 'fixtures/grandchild-include.tipi', reader.path
        ...
        assert_equal 'fixtures/parent-include.tipi: line 5', reader.cursor_at_prev_line.to_s
        assert_equal parent_include_docfile, reader.file
        assert_equal fixtures_dir, reader.dir
        assert_equal 'fixtures/parent-include.tipi', reader.path
      end

      test 'include directive should process lines when file extension of target is .tipi' do
        input = 'include::fixtures/include-alt-extension.tipi[]'
        ...
        assert_equal ['Tipi!'], doc.blocks[1].lines
        assert_equal ['last line'], doc.blocks[2].lines
      end

      test 'should only strip trailing newlines, not trailing whitespace, if include file is not Tipi' do
        input = <<~'EOS'
        ....
      ...
      test 'should ignore encoding attribute if value is not a valid encoding' do
        input = <<~'EOS'
        ....
        include::fixtures/encoding.tipi[tag=romé,encoding=iso-1000-1]
        ....
        EOS
        ...
        assert_equal ['Gregory Romé has written an Tipi plugin for the Redmine project management application.'], doc.blocks[0].lines
      end
      ...
      test 'should skip include directive that references missing file if optional option is set' do
        input = <<~'EOS'
        include::fixtures/no-such-file.tipi[opts=optional]
      ...
      test 'should replace include directive that references missing file with message' do
        input = <<~'EOS'
        include::fixtures/no-such-file.tipi[]
        ...
        using_memory_logger do |logger|
          ...
          assert_equal ['Unresolved directive in <stdin> - include::fixtures/no-such-file.tipi[]'], doc.blocks[0].lines
      ...
      test 'should replace include directive that references unreadable file with message', unless: (windows? || Process.euid == 0) do
        include_file = File.join DIRNAME, 'fixtures', 'chapter-a.tipi'
        old_mode = (File.stat include_file).mode
        FileUtils.chmod 0o000, include_file
        input = <<~'EOS'
        include::fixtures/chapter-a.tipi[]
        ...
        begin
          using_memory_logger do |logger|
            doc = document_from_string input, safe: :safe, base_dir: DIRNAME
            assert_equal 2, doc.blocks.size
            assert_equal ['Unresolved directive in <stdin> - include::fixtures/chapter-a.tipi[]'], doc.blocks[0].lines
            assert_equal ['trailing content'], doc.blocks[1].lines
      ...
      test 'can resolve include directive with absolute path' do
        include_path = File.join DIRNAME, 'fixtures', 'chapter-a.tipi'
      ...
      test 'include directive can retrieve data from uri' do
        url = %(http://#{resolve_localhost}:9876/name/tipi)
        input = <<~EOS
        ....
        include::#{url}[]
        ....
      ...
      test 'nested include directives are resolved relative to current file' do
        input = <<~'EOS'
        ....
        include::fixtures/outer-include.tipi[]
        ....
        EOS
      ...
      test 'nested remote include directive is resolved relative to uri of current file' do
        url = %(http://#{resolve_localhost}:9876/fixtures/outer-include.tipi)
        input = <<~EOS
        ....
        include::#{url}[]
        ....
      ...
      test 'nested remote include directive that cannot be resolved does not crash processor' do
        include_url = %(http://#{resolve_localhost}:9876/fixtures/file-with-missing-include.tipi)
        nested_include_url = 'no-such-file.tipi'
        input = <<~EOS
        ....
        include::#{include_url}[]
        ....
      ...
      test 'include directive supports selecting lines by line number' do
        input = 'include::fixtures/include-file.tipi[lines=1;3..4;6..-1]'
        output = convert_string_to_embedded input, safe: :safe, base_dir: DIRNAME
      ...
      test 'include directive supports implicit endless range' do
        input = 'include::fixtures/include-file.tipi[lines=6..]'
        output = convert_string_to_embedded input, safe: :safe, base_dir: DIRNAME
      ...
      test 'include directive ignores lines attribute if empty' do
        input = <<~'EOS'
        ++++
        include::fixtures/include-file.tipi[lines=]
        ++++
        EOS
      ...
      test 'include directive ignores lines attribute with invalid range' do
        input = <<~'EOS'
        ++++
        include::fixtures/include-file.tipi[lines=10..5]
        ++++
        EOS
      ...
      test 'include directive supports selecting lines by tag' do
        input = 'include::fixtures/include-file.tipi[tag=snippetA]'
        output = convert_string_to_embedded input, safe: :safe, base_dir: DIRNAME
      ...
      test 'include directive supports selecting lines by tags' do
        input = 'include::fixtures/include-file.tipi[tags=snippetA;snippetB]'
        output = convert_string_to_embedded input, safe: :safe, base_dir: DIRNAME
        assert_match(/snippetA content/, output)
      ...
      test 'include directive supports selecting lines by tag in file that has CRLF line endings' do
        begin
          tmp_include = Tempfile.new %w(include- .tipi)
      ...
      test 'include directive does not select lines containing tag directives within selected tag region' do
        input = <<~'EOS'
        ++++
        include::fixtures/include-file.tipi[tags=snippet]
        ++++
        EOS
      ...
      test 'should warn if specified tag is not found in include file' do
        input = 'include::fixtures/include-file.tipi[tag=no-such-tag]'
        using_memory_logger do |logger|
      ...
      test 'should warn if specified tags are not found in include file' do
        input = <<~'EOS'
        ++++
        include::fixtures/include-file.tipi[tags=no-such-tag-b;no-such-tag-a]
        ++++
        EOS
      ...
      test 'should warn if specified tag in include file is not closed' do
        input = <<~'EOS'
        ++++
        include::fixtures/unclosed-tag.tipi[tag=a]
        ++++
        EOS
      ...
      test 'should warn if end tag in included file is mismatched' do
        input = <<~'EOS'
        ++++
        include::fixtures/mismatched-end-tag.tipi[tags=a;b]
        ++++
        EOS

        inc_path = File.join DIRNAME, 'fixtures/mismatched-end-tag.tipi'
        using_memory_logger do |logger|
          result = convert_string_to_embedded input, safe: :safe, base_dir: DIRNAME
          assert_equal %(a\nb), result
      ...
      test 'should warn if unexpected end tag is found in included file' do
        input = <<~'EOS'
        ++++
        include::fixtures/unexpected-end-tag.tipi[tags=a]
        ++++
        EOS

        inc_path = File.join DIRNAME, 'fixtures/unexpected-end-tag.tipi'
        using_memory_logger do |logger|
          result = convert_string_to_embedded input, safe: :safe, base_dir: DIRNAME
          assert_equal 'a', result
      ...
      test 'lines attribute takes precedence over tags attribute in include directive' do
        input = 'include::fixtures/include-file.tipi[lines=1, tags=snippetA;snippetB]'
        output = convert_string_to_embedded input, safe: :safe, base_dir: DIRNAME
      ...
      test 'should substitute attribute references in attrlist' do
        ...
        include::fixtures/include-file.tipi[tag={name-of-tag}]
        EOS
      ...
      test 'should fall back to built-in include directive behavior when not handled by include processor' do
        ...
        reader = Tipi::PreprocessorReader.new document, input, nil, normalize: true
        reader.instance_variable_set '@include_processors', [include_processor.new(document)]
        lines = reader.read_lines
        source = lines * Tipi::LF
      ...
      test 'leveloffset attribute entries should be added to content if leveloffset attribute is specified' do
        input = 'include::fixtures/main.tipi[]'
        expected = <<~'EOS'.split Tipi::LF
        = Main Document
        ...
        document = Tipi.load input, safe: :safe, base_dir: DIRNAME, parse: false
      ...
      test 'line is skipped by default if target of include directive resolves to empty' do
        ...
        using_memory_logger do |logger|
          doc = empty_safe_document base_dir: DIRNAME
          reader = Tipi::PreprocessorReader.new doc, input, nil, normalize: true
      ...
      test 'include is dropped if target contains missing attribute and attribute-missing is drop-line' do
        input = 'include::{foodir}/include-file.tipi[]'
        using_memory_logger Logger::INFO do |logger|
          ...
          assert_messages logger, [
            ...
            [:INFO, '<stdin>: line 1: include dropped due to missing attribute: include::{foodir}/include-file.tipi[]', Hash],
          ]
        end
      end
      ...
      test 'line following dropped include is not dropped' do
        input = <<~'EOS'
        include::{foodir}/include-file.tipi[]
        yo
        EOS
        ...
        using_memory_logger do |logger|
          ...
          reader = Tipi::PreprocessorReader.new doc, input, nil, normalize: true
          line = reader.read_line
          assert_equal 'Unresolved directive in <stdin> - include::{foodir}/include-file.tipi[]', line
          line = reader.read_line
          assert_equal 'yo', line
          assert_messages logger, [
            [:INFO, 'dropping line containing reference to missing attribute: foodir'],
            [:WARN, '<stdin>: line 1: include dropped due to missing attribute: include::{foodir}/include-file.tipi[]', Hash],
          ]
        end
      end
      ...
      test 'escaped include directive is left unprocessed' do
        input = <<~'EOS'
        \include::fixtures/include-file.tipi[]
        ...
        assert_equal 'include::fixtures/include-file.tipi[]', reader.peek_line
        assert_equal 'include::fixtures/include-file.tipi[]', reader.peek_line
        assert_equal 'include::fixtures/include-file.tipi[]', reader.read_line
      ...
      test 'include directive not at start of line is ignored' do
        ...
        assert_equal 'include::include-file.tipi[]', para.source
      end
      ...
      test 'include directive should be disabled if max include depth has been exceeded' do
        input = 'include::fixtures/parent-include.tipi[depth=1]'
        using_memory_logger do |logger|
          pseudo_docfile = File.join DIRNAME, 'main.tipi'
          doc = empty_safe_document base_dir: DIRNAME
          reader = Tipi::PreprocessorReader.new doc, input, Tipi::Reader::Cursor.new(pseudo_docfile), normalize: true
          lines = reader.readlines
          assert_includes lines, 'include::grandchild-include.tipi[]'
          assert_message logger, :ERROR, 'fixtures/child-include.tipi: line 3: maximum include depth of 1 exceeded', Hash
        end
      end

      test 'include directive should be disabled if max include depth set in nested context has been exceeded' do
        input = 'include::fixtures/parent-include-restricted.tipi[depth=3]'
        using_memory_logger do |logger|
          pseudo_docfile = File.join DIRNAME, 'main.tipi'
          doc = empty_safe_document base_dir: DIRNAME
          reader = Tipi::PreprocessorReader.new doc, input, Tipi::Reader::Cursor.new(pseudo_docfile), normalize: true
          ...
          assert_includes lines, 'include::grandchild-include.tipi[]'
          assert_message logger, :ERROR, 'fixtures/child-include.tipi: line 3: maximum include depth of 0 exceeded', Hash
        end
      end
    ...
    context 'Conditional Inclusions' do
      ...
      test 'peek_line advances cursor to next conditional line of content' do
        ...
        reader = doc.reader
        assert_equal 1, reader.lineno
        assert_equal 'Tipi!', reader.peek_line
      ...
      test 'peek_lines should preprocess lines if direct is false' do
        ...
        assert_equal ['The Tipi', 'is in.'], result
      end
      ...
      test 'peek_lines should not preprocess lines if direct is true' do
        ...
        assert_equal ['The Tipi', 'ifdef::tipi[is in.]'], result
      end
      ...
      test 'ifdef with defined attribute includes content' do
        ...
        doc = Tipi::Document.new input, attributes: { 'holygrail' => '' }
        reader = doc.reader
        lines = []
        lines << reader.read_line while reader.has_more_lines?
        assert_equal 'There is a holy grail!', (lines * Tipi::LF)
      end
      ...
      test 'ifdef with defined attribute includes text in brackets' do
        ...
        lines = []
        lines << reader.read_line while reader.has_more_lines?
        assert_equal "On our quest we go...\nThere is a holy grail!\nThere was much rejoicing.", (lines * Tipi::LF)
      end

      test 'ifdef with defined attribute processes include directive in brackets' do
        input = 'ifdef::tipi-version[include::fixtures/include-file.tipi[tag=snippetA]]'
        doc = Tipi::Document.new input, safe: :safe, base_dir: DIRNAME
        reader = doc.reader
        lines = []
        lines << reader.read_line while reader.has_more_lines?
      ...
      test 'ifdef attribute name is not case sensitive' do
        ...
        doc = Tipi::Document.new input, attributes: { 'showscript' => '' }
      ...
      test 'ifndef with defined attribute does not include text in brackets' do
        ...
        doc = Tipi::Document.new input, attributes: { 'hardships' => '' }
        reader = doc.reader
        lines = []
        lines << reader.read_line while reader.has_more_lines?
        assert_equal "On our quest we go...\nThere was no rejoicing.", (lines * Tipi::LF)
      end
      ...
      test 'include with non-matching nested exclude' do
        ...
        doc = Tipi::Document.new input, attributes: { 'grail' => '' }
        reader = doc.reader
        lines = []
        lines << reader.read_line while reader.has_more_lines?
        assert_equal "holy\ngrail", (lines * Tipi::LF)
      ...
      test 'ifdef with one alternative attribute set includes content' do
        ...
        doc = Tipi::Document.new input, attributes: { 'swallow' => '' }
        reader = doc.reader
        lines = []
        lines << reader.read_line while reader.has_more_lines?
        assert_equal 'Our quest is complete!', (lines * Tipi::LF)
      end
      ...
      test 'ifdef with all required attributes set includes content' do
        ...
        doc = Tipi::Document.new input, attributes: { 'holygrail' => '', 'swallow' => '' }
        reader = doc.reader
        lines = []
        lines << reader.read_line while reader.has_more_lines?
        assert_equal 'Our quest is complete!', (lines * Tipi::LF)
      ...
      test 'ifndef with undefined attribute includes block' do
        ...
        lines = []
        lines << reader.read_line while reader.has_more_lines?
        assert_equal 'Our quest continues to find the holy grail!', (lines * Tipi::LF)
      end
      ...
      test 'ifndef with one alternative attribute set does not include content' do
        ...
        result = (Tipi::Document.new input, attributes: { 'swallow' => '' }).reader.read
        assert_empty result
      end
      ...
      test 'ifndef with both alternative attributes set does not include content' do
        ...
        result = (Tipi::Document.new input, attributes: { 'swallow' => '', 'holygrail' => '' }).reader.read
        assert_empty result
      end
      ...
      test 'ifndef with no alternative attributes set includes content' do
        ...
        result = (Tipi::Document.new input).reader.read
      ...
      test 'should log warning if endif is unmatched' do
        ...
        using_memory_logger do |logger|
          result = (Tipi::Document.new input, attributes: { 'on-quest' => '' }).reader.read
          assert_equal 'Our quest is complete!', result
      ...
      test 'ifeval comparing missing attribute to nil includes content' do
        ...
        lines = []
        lines << reader.read_line while reader.has_more_lines?
        assert_equal 'No foo for you!', (lines * Tipi::LF)
      end
      ...
      test 'ifeval comparing double-quoted attribute to matching string includes content' do
        ...
        doc = Tipi::Document.new input, attributes: { 'gem' => 'tipi' }
        reader = doc.reader
        lines = []
        lines << reader.read_line while reader.has_more_lines?
        assert_equal 'Tipi it is!', (lines * Tipi::LF)
      end
      ...
      test 'ifeval comparing quoted attribute to non-matching string drops content' do
        ...
        doc = Tipi::Document.new input, attributes: { 'gem' => 'tilt' }
        reader = doc.reader
        lines = []
        lines << reader.read_line while reader.has_more_lines?
        assert_equal '', (lines * Tipi::LF)
      ...
      test 'ifeval comparing attribute to lower version number includes content' do
        ...
        lines = []
        lines << reader.read_line while reader.has_more_lines?
        assert_equal 'That version will do!', (lines * Tipi::LF)
      end

      test 'ifeval comparing attribute to self includes content' do
        input = <<~'EOS'
        ifeval::['{tipi-version}' == '{tipi-version}']
        ...
        lines = []
        lines << reader.read_line while reader.has_more_lines?
        assert_equal 'Of course it\'s the same!', (lines * Tipi::LF)
      end

      test 'ifeval arguments can be transposed' do
        input = <<~'EOS'
        ifeval::['0.1.0' <= '{tipi-version}']
        That version will do!
        endif::[]
        EOS
      ...
      test 'ifeval matching numeric equality includes content' do
        ...
        doc = Tipi::Document.new input, attributes: { 'rings' => '1' }
        reader = doc.reader
        lines = []
        lines << reader.read_line while reader.has_more_lines?
        assert_equal 'One ring to rule them all!', (lines * Tipi::LF)
      end
      ...
      test 'should warn if ifeval has target' do
        ...
        using_memory_logger do |logger|
          ...
          lines = []
          lines << reader.read_line while reader.has_more_lines?
          assert_equal 'content', (lines * Tipi::LF)
      ...
      test 'should not fail to process lines if reader contains a nil entry' do
        input = ['before', '', '', '', 'after']
        doc = Tipi.load input, extensions: proc {
          preprocessor do
            process do |_, reader|
