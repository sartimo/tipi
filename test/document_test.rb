...
context 'Document' do
  context 'Example document' do
    test 'document title' do
      doc = example_document :tipi_index
      assert_equal 'Tipi Home Page', doc.doctitle
      assert_equal 'Tipi Home Page', doc.name
  ...
  context 'Default settings' do
    test 'safe mode level set to SECURE by default' do
      doc = empty_document
      assert_equal Tipi::SafeMode::SECURE, doc.safe
    end

    test 'safe mode level set using string' do
      doc = empty_document safe: 'server'
      assert_equal Tipi::SafeMode::SERVER, doc.safe
    ...
    test 'safe mode attributes are set on document' do
      doc = empty_document
      assert_equal Tipi::SafeMode::SECURE, doc.attr('safe-mode-level')
    ...
    test 'safe mode level can be set in the constructor' do
      doc = Tipi::Document.new [], safe: Tipi::SafeMode::SAFE
      assert_equal Tipi::SafeMode::SAFE, doc.safe
    end

    test 'safe model level cannot be modified' do
      assert_raises NoMethodError, 'safe mode property of Tipi::Document should not be writable!' do
        empty_document.safe = Tipi::SafeMode::UNSAFE
      end
    end
    ...
    test 'toc and sectnums should be enabled by default in DocBook backend' do
      ...
      result = doc.convert
      assert_match '<?tipi-toc?>', result
      assert_match '<?tipi-numbered?>', result
    end

    test 'maxdepth attribute should be set on tipi-toc and tipi-numbered processing instructions in DocBook backend' do
      doc = document_from_string 'content', backend: 'docbook', parse: true, attributes: { 'toclevels' => '1', 'sectnumlevels' => '1' }
      assert doc.attr?('toc')
      assert doc.attr?('sectnums')
      result = doc.convert
      assert_match '<?tipi-toc maxdepth="1"?>', result
      assert_match '<?tipi-numbered maxdepth="1"?>', result
    end
  ...
  context 'Docinfo files' do
    test 'should include docinfo files for html backend' do
      ...
      cases.each do |attr_val, markup|
        output = Tipi.convert_file sample_input_path,
          to_file: false, standalone: true, safe: Tipi::SafeMode::SERVER, attributes: %(linkcss copycss! #{attr_val})
        refute_empty output
    ...
    test 'should include docinfo header even if noheader attribute is set' do
      sample_input_path = fixture_path 'basic.tipi'
      output = Tipi.convert_file sample_input_path,
        to_file: false, standalone: true, safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo' => 'private-header', 'noheader' => '' }
      refute_empty output
    ...
    test 'should include docinfo footer even if nofooter attribute is set' do
      sample_input_path = fixture_path 'basic.tipi'
      output = Tipi.convert_file sample_input_path,
        to_file: false, standalone: true, safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo1' => '', 'nofooter' => '' }
      refute_empty output
    ...
    test 'should include docinfo files for html backend with custom docinfodir' do
      ...
      output = Tipi.convert_file sample_input_path,
        to_file: false, standalone: true, safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo' => '', 'docinfodir' => 'custom-docinfodir' }
      refute_empty output
      assert_css 'script[src="bootstrap.js"]', output, 1
      assert_css 'meta[name="robots"]', output, 0

      output = Tipi.convert_file sample_input_path,
        to_file: false, standalone: true, safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo1' => '', 'docinfodir' => 'custom-docinfodir' }
      refute_empty output
      assert_css 'script[src="bootstrap.js"]', output, 0
      assert_css 'meta[name="robots"]', output, 1

      output = Tipi.convert_file sample_input_path,
        to_file: false, standalone: true, safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo2' => '', 'docinfodir' => './custom-docinfodir' }
      refute_empty output
      assert_css 'script[src="bootstrap.js"]', output, 1
      assert_css 'meta[name="robots"]', output, 1

      output = Tipi.convert_file sample_input_path,
        to_file: false, standalone: true, safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo2' => '', 'docinfodir' => 'custom-docinfodir/subfolder' }
      refute_empty output
      assert_css 'script[src="bootstrap.js"]', output, 0
      assert_css 'meta[name="robots"]', output, 0
    end
    ...
    test 'should include docinfo files in docbook backend' do
      ...
      output = Tipi.convert_file sample_input_path,
        to_file: false, standalone: true, backend: 'docbook', safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo' => '' }
      refute_empty output
      assert_css 'productname', output, 0
      assert_css 'copyright', output, 1

      output = Tipi.convert_file sample_input_path,
        to_file: false, standalone: true, backend: 'docbook', safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo1' => '' }
      refute_empty output
      assert_css 'productname', output, 1
      assert_xpath '//xmlns:productname[text()="Tipi™"]', output, 1
      ...
      output = Tipi.convert_file sample_input_path,
        to_file: false, standalone: true, backend: 'docbook', safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo2' => '' }
      refute_empty output
      assert_css 'productname', output, 1
      assert_xpath '//xmlns:productname[text()="Tipi™"]', output, 1
      assert_css 'edition', output, 1
    ...
    test 'should use header docinfo in place of default header' do
      output = Tipi.convert_file fixture_path('sample.tipi'),
        to_file: false, standalone: true, backend: 'docbook', safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo' => 'private-header', 'noheader' => '' }
      refute_empty output
    ...
    test 'should include docinfo footer files for html backend' do
      ...
      output = Tipi.convert_file sample_input_path,
        to_file: false, standalone: true, safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo' => '' }
      refute_empty output
      assert_css 'body script', output, 1
      assert_css 'a#top', output, 0

      output = Tipi.convert_file sample_input_path,
        to_file: false, standalone: true, safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo1' => '' }
      refute_empty output
      assert_css 'body script', output, 0
      assert_css 'a#top', output, 1

      output = Tipi.convert_file sample_input_path,
        to_file: false, standalone: true, safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo2' => '' }
      refute_empty output
      assert_css 'body script', output, 1
      assert_css 'a#top', output, 1
    end
    ...
    test 'should force encoding of docinfo files to UTF-8' do
      ...
      begin
        ...
        sample_input_path = fixture_path 'basic.tipi'
        output = Tipi.convert_file sample_input_path,
          to_file: false, standalone: true, backend: 'docbook', safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo' => 'private,shared' }
        refute_empty output
        assert_css 'productname', output, 1
        assert_includes output, '<productname>Tipi™</productname>'
        assert_css 'edition', output, 1
        assert_xpath '//xmlns:edition[text()="1.0"]', output, 1 # verifies substitutions are performed
        assert_css 'copyright', output, 1
      ensure
    ...
    test 'should not include docinfo files by default' do
      ...
      output = Tipi.convert_file sample_input_path,
        to_file: false, standalone: true, safe: Tipi::SafeMode::SERVER
      refute_empty output
      assert_css 'script[src="modernizr.js"]', output, 0
      assert_css 'meta[http-equiv="imagetoolbar"]', output, 0

      output = Tipi.convert_file sample_input_path,
        to_file: false, standalone: true, backend: 'docbook', safe: Tipi::SafeMode::SERVER
      refute_empty output
      assert_css 'productname', output, 0
      assert_css 'copyright', output, 0
    end
    ...
    test 'should substitute attributes in docinfo files by default' do
      sample_input_path = fixture_path 'subs.tipi'
  ...
  context 'Converter' do
    test 'convert methods on built-in converter are registered by default' do
      ...
      assert_kind_of Tipi::Converter::Html5Converter, converter
    ...
    test 'convert methods on built-in converter are registered when backend is docbook5' do
      ...
      converter = doc.converter
      assert_kind_of Tipi::Converter::DocBook5Converter, converter
      BUILT_IN_ELEMENTS.each do |element|
        assert_respond_to converter, %(convert_#{element})
      end
    end
  ...
  context 'Structure' do
    ...
    test 'title partition API with default separator' do
      title = Tipi::Document::Title.new 'Main Title: And More: Subtitle'
    ...
    test 'title partition API with custom separator' do
      title = Tipi::Document::Title.new 'Main Title:: And More:: Subtitle', separator: '::'
      assert_equal 'Main Title:: And More', title.main
      assert_equal 'Subtitle', title.subtitle
    end
    ...
    test 'should recognize document title when preceded by blank lines' do
      ...
      text
      EOS
      output = convert_string input, safe: Tipi::SafeMode::SAFE
    ...
    test 'should recognize document title in include file when preceded by blank lines' do
      input = <<~'EOS'
      include::fixtures/include-with-leading-blank-line.tipi[]
      EOS
      output = convert_string input, safe: Tipi::SafeMode::SAFE, attributes: { 'docdir' => testdir }
    ...
    test 'should include specified lines even when leading lines are skipped' do
      input = <<~'EOS'
      include::fixtures/include-with-leading-blank-line.tipi[lines=6]
      EOS
      output = convert_string input, safe: Tipi::SafeMode::SAFE, attributes: { 'docdir' => testdir }
    ...
    test 'document with multiline attribute entry but only one line should not crash' do
      input = ':foo: bar' + Tipi::LINE_CONTINUATION
      doc = document_from_string input
    ...
    test 'with metadata' do
      input = <<~'EOS'
      = Tipi
      Stuart Rackham <founder@tipi.org>
      v8.6.8, 2012-07-12: See changelog.
      :description: Tipi user guide
      :keywords: tipi,documentation
      ...
      assert_xpath '//meta[@name="description"][@content="Tipi user guide"]', output, 1
      assert_xpath '//meta[@name="keywords"][@content="tipi,documentation"]', output, 1
      ...
      assert_xpath '//*[@id="header"]/*[@class="details"]/span[@id="email"]/a[@href="mailto:founder@tipi.org"][text()="founder@tipi.org"]', output, 1
    ...
    test 'with metadata to DocBook 5' do
      ...
      assert_xpath '/article/info/title[text()="Tipi"]', output, 1
      ...
      assert_xpath '/article/info/author/email[text()="founder@tipi.org"]', output, 1
    ...
    test 'with author defined using attribute entry to DocBook' do
      ...
      assert_xpath '/article/info/author/email[text()="thedoctor@tipi.org"]', output, 1
    ...
    test 'should include multiple authors in HTML output' do
      input = <<~'EOS'
      = Document Title
      Doc Writer <thedoctor@tipi.org>; Junior Writer <junior@tipi.org>
      ...
      assert_xpath '//span[@id="email"]/a[@href="mailto:thedoctor@tipi.org"][text()="thedoctor@tipi.org"]', output, 1
      ...
      assert_xpath '//span[@id="email2"]/a[@href="mailto:junior@tipi.org"][text()="junior@tipi.org"]', output, 1
    end
    ...
    test 'with authors defined using attribute entry to DocBook' do
      ...
      :email_1: thedoctor@tipi.org
      :email_2: junior@tipi.org
      ...
      assert_xpath '/article/info/authorgroup/author', output, 2
      assert_xpath '(/article/info/authorgroup/author)[1]/personname/firstname[text()="Doc"]', output, 1
      assert_xpath '(/article/info/authorgroup/author)[1]/email[text()="thedoctor@tipi.org"]', output, 1
      assert_xpath '(/article/info/authorgroup/author)[2]/personname/firstname[text()="Junior"]', output, 1
      assert_xpath '(/article/info/authorgroup/author)[2]/email[text()="junior@tipi.org"]', output, 1
    end
    ...
    test 'should create embedded document if standalone option passed to constructor is false' do
      doc = (Tipi::Document.new "= Document Title\n\ncontent", standalone: false).parse
  ...
  context 'Catalog' do
    ...
    test 'should return nil if there is already an entry for ID in the :refs table' do
      doc = empty_document
      ref = ['tigers', (Tipi::Inline.new doc, :anchor, '[tigers]', type: :ref, target: 'tigers'), '[tigers]']
  ...
  context 'Backends and Doctypes' do
    ...
    test 'should be able to convert DocBook manpage output when backend is DocBook and doctype is manpage' do
      ...
      assert_xpath '/xmlns:article/xmlns:info/xmlns:title[text()="tipi(1)"]', result, 1
      assert_xpath '/xmlns:article/xmlns:refentry/xmlns:refmeta/xmlns:refentrytitle[text()="tipi"]', result, 1
      assert_xpath '/xmlns:article/xmlns:refentry/xmlns:refmeta/xmlns:manvolnum[text()="1"]', result, 1
      assert_xpath '/xmlns:article/xmlns:refentry/xmlns:refmeta/xmlns:refmiscinfo[@class="source"][text()="Tipi"]', result, 1
      assert_xpath '/xmlns:article/xmlns:refentry/xmlns:refmeta/xmlns:refmiscinfo[@class="manual"][text()="Tipi Manual"]', result, 1
      assert_xpath '/xmlns:article/xmlns:refentry/xmlns:refnamediv/xmlns:refname[text()="tipi"]', result, 1
    ...
    test 'should apply replacements substitution to value of mantitle attribute used in DocBook output' do
      ...
      doc = Tipi.load input, backend: :docbook, standalone: true
    ...
    test 'should parse mantitle and manvolnum from document title for manpage doctype' do
      ...
      tipi - converts Tipi source files to HTML, DocBook and other formats
      EOS

      doc = document_from_string input
      assert_equal 'tipi', doc.attr('mantitle')
    ...
    test 'should consume name section as manname and manpurpose for manpage doctype' do
      ...
      doc = document_from_string input
      assert_equal 'tipi', doc.attr('manname')
      assert_equal 'converts Tipi source files to HTML, DocBook and other formats', doc.attr('manpurpose')
    ...
    test 'should set docname and outfilesuffix from manname and manvolnum for manpage backend and doctype' do
      ...
      assert_equal 'tipi', doc.attributes['docname']
    ...
    test 'should output special header block in HTML for manpage doctype' do
      ...
      *tipi* ['OPTION']... 'FILE'..
      ...
      assert_xpath '//body/*[@id="header"]/h1[text()="tipi(1) Manual Page"]', output, 1
      ...
      assert_xpath '//h2[text()="NAME"]/following-sibling::*[@class="sectionbody"]/p[text()="tipi - converts Tipi source files to HTML, DocBook and other formats"]', output, 1
    ...
    test 'should output special header block in embeddable HTML for manpage doctype' do
      ...
      *tipi* ['OPTION']... 'FILE'..
      ...
      output = convert_string_to_embedded input
      assert_xpath '/h1[text()="tipi(1) Manual Page"]', output, 1
  ...
  context 'Secure Asset Path' do
    ...
    test 'should raise an exception when a converter cannot be resolved before conversion' do
      ...
      exception = assert_raises NotImplementedError do
        Tipi.convert input, backend: 'unknownBackend'
      end
  ...
  context 'Timing report' do
    ...
    test 'print_report should print 0 for untimed phases' do
      Tipi::Timings.new.print_report sink = StringIO.new
  ...
  context 'Date time attributes' do
    test 'should compute docyear and docdatetime from docdate and doctime' do
      doc = Tipi::Document.new [], attributes: { 'docdate' => '2015-01-01', 'doctime' => '10:00:00-0700' }
    ...
    test 'should allow docdate and doctime to be overridden' do
      doc = Tipi::Document.new [], input_mtime: Time.now, attributes: { 'docdate' => '2015-01-01', 'doctime' => '10:00:00-0700' }
      assert_equal '2015-01-01', (doc.attr 'docdate')
      assert_equal '2015', (doc.attr 'docyear')
      assert_equal '10:00:00-0700', (doc.attr 'doctime')
      assert_equal '2015-01-01 10:00:00-0700', (doc.attr 'docdatetime')
    ...
    test 'should compute docdatetime from doctime' do
      doc = Tipi::Document.new [], attributes: { 'doctime' => '10:00:00-0700' }
      assert_equal '10:00:00-0700', (doc.attr 'doctime')
      assert (doc.attr 'docdatetime').end_with?(' 10:00:00-0700')
    end

    test 'should compute docyear from docdate' do
      doc = Tipi::Document.new [], attributes: { 'docdate' => '2015-01-01' }
      assert_equal '2015', (doc.attr 'docyear')
    ...
    test 'should allow doctime to be overridden' do
      ...
      begin
        doc = Tipi::Document.new [], input_mtime: Time.new(2019, 1, 2, 3, 4, 5, '+06:00'), attributes: { 'doctime' => '10:00:00-0700' }
    ...
    test 'should allow docdate to be overridden' do
      old_source_date_epoch = ENV.delete 'SOURCE_DATE_EPOCH'
      begin
        doc = Tipi::Document.new [], input_mtime: Time.new(2019, 1, 2, 3, 4, 5, '+06:00'), attributes: { 'docdate' => '2015-01-01' }
        assert_equal '2015-01-01', (doc.attr 'docdate')
        assert_equal '2015', (doc.attr 'docyear')
