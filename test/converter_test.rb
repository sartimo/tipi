...
context 'Converter' do
  context 'View options' do
    test 'should set Haml format to html5 for html5 backend' do
      doc = Tipi::Document.new [], template_dir: (fixture_path 'custom-backends/haml'), template_cache: false
      assert_kind_of Tipi::Converter::CompositeConverter, doc.converter
      selected = doc.converter.find_converter 'paragraph'
      assert_kind_of Tipi::Converter::TemplateConverter, selected
    ...
    test 'should set Haml format to xhtml for docbook backend' do
      doc = Tipi::Document.new [], backend: 'docbook5', template_dir: (fixture_path 'custom-backends/haml'), template_cache: false
      assert_kind_of Tipi::Converter::CompositeConverter, doc.converter
      selected = doc.converter.find_converter 'paragraph'
      assert_kind_of Tipi::Converter::TemplateConverter, selected
      assert_kind_of haml_template_class, selected.templates['paragraph']
    ...
    test 'should configure Slim to resolve includes in specified template dirs' do
      ...
      doc = Tipi::Document.new [], template_dirs: template_dirs, template_cache: false
      assert_kind_of Tipi::Converter::CompositeConverter, doc.converter
      selected = doc.converter.find_converter 'paragraph'
      assert_kind_of Tipi::Converter::TemplateConverter, selected
    ...
    test 'should set Slim format to html for html5 backend' do
      doc = Tipi::Document.new [], template_dir: (fixture_path 'custom-backends/slim'), template_cache: false
      assert_kind_of Tipi::Converter::CompositeConverter, doc.converter
      selected = doc.converter.find_converter 'paragraph'
      assert_kind_of Tipi::Converter::TemplateConverter, selected
      assert_kind_of Slim::Template, selected.templates['paragraph']
    ...
    test 'should set Slim format to nil for docbook backend' do
      doc = Tipi::Document.new [], backend: 'docbook5', template_dir: (fixture_path 'custom-backends/slim'), template_cache: false
      assert_kind_of Tipi::Converter::CompositeConverter, doc.converter
      selected = doc.converter.find_converter 'paragraph'
      assert_kind_of Tipi::Converter::TemplateConverter, selected
      assert_kind_of Slim::Template, selected.templates['paragraph']
    ...
    test 'should set safe mode of Slim Tipi engine to match document safe mode when Slim >= 3' do
      doc = Tipi::Document.new [], template_dir: (fixture_path 'custom-backends/slim'), template_cache: false, safe: :unsafe
      assert_kind_of Tipi::Converter::CompositeConverter, doc.converter
      selected = doc.converter.find_converter 'paragraph'
      assert_kind_of Tipi::Converter::TemplateConverter, selected
      slim_tipi_opts = selected.instance_variable_get(:@engine_options)[:slim][:tipi]
      assert_equal({ safe: Tipi::SafeMode::UNSAFE }, slim_tipi_opts)
    end

    test 'should support custom template engine options for known engine' do
      doc = Tipi::Document.new [], template_dir: (fixture_path 'custom-backends/slim'), template_cache: false, template_engine_options: { slim: { pretty: true } }
      assert_kind_of Tipi::Converter::CompositeConverter, doc.converter
      selected = doc.converter.find_converter 'paragraph'
      assert_kind_of Tipi::Converter::TemplateConverter, selected
      assert_kind_of Slim::Template, selected.templates['paragraph']
  ...
  context 'Custom backends' do
    ...
    test 'should set outfilesuffix according to backend info' do
      doc = Tipi.load 'content'
      doc.convert
      assert_equal '.html', doc.attributes['outfilesuffix']

      doc = Tipi.load 'content', template_dir: (fixture_path 'custom-backends/haml'), template_cache: false
      doc.convert
      assert_equal '.html', doc.attributes['outfilesuffix']
    end

    test 'should not override outfilesuffix attribute if locked' do
      doc = Tipi.load 'content', attributes: { 'outfilesuffix' => '.foo' }
      doc.convert
      assert_equal '.foo', doc.attributes['outfilesuffix']

      doc = Tipi.load 'content', template_dir: (fixture_path 'custom-backends/haml'), template_cache: false, attributes: { 'outfilesuffix' => '.foo' }
      doc.convert
      assert_equal '.foo', doc.attributes['outfilesuffix']
    end
    ...
    test 'should allow custom backend to emulate a known backend' do
      doc = Tipi.load 'content', backend: 'html5-tweaks:html', template_dir: (fixture_path 'custom-backends/haml'), template_cache: false
      assert doc.basebackend? 'html'
      assert_equal 'html5-tweaks', doc.backend
      converter = doc.converter
      assert_kind_of Tipi::Converter::TemplateConverter, (converter.find_converter 'embedded')
      refute_kind_of Tipi::Converter::TemplateConverter, (converter.find_converter 'admonition')
    ...
    test 'should use built-in global cache to cache templates' do
      begin
        Tipi::Converter::TemplateConverter.clear_caches if defined? Tipi::Converter::TemplateConverter
        template_dir = fixture_path 'custom-backends/haml'
        doc = Tipi::Document.new [], template_dir: template_dir
        doc.converter
        caches = Tipi::Converter::TemplateConverter.caches
        ...
        doc = Tipi::Document.new [], template_dir: template_dir, template_cache: false
        template_converter = doc.converter.find_converter 'paragraph'
        paragraph_template_after = template_converter.templates['paragraph']
        refute_nil paragraph_template_after
    ...
    test 'should use custom cache to cache templates' do
      template_dir = fixture_path 'custom-backends/haml'
      Tipi::PathResolver.new.system_path File.join(template_dir, 'html5', 'block_paragraph.html.haml'), nil
      caches = { scans: {}, templates: {} }
      doc = Tipi::Document.new [], template_dir: template_dir, template_cache: caches
      doc.converter
    ...
    test 'should load ERB templates using ERBTemplate if eruby is not set' do
      input = %([.wrapper]\n--\nfoobar\n--)
      doc = Tipi::Document.new input, template_dir: (fixture_path 'custom-backends/erb'), template_cache: false
      assert_kind_of Tipi::Converter::CompositeConverter, doc.converter
      %w(paragraph).each do |node_name|
        selected = doc.converter.find_converter node_name
        assert_kind_of Tipi::Converter::TemplateConverter, selected
    ...
    test 'should load ERB templates using ErubiTemplate if eruby is set to erubi' do
      doc = Tipi::Document.new [], template_dir: (fixture_path 'custom-backends/erb'), template_cache: false, eruby: 'erubi'
      assert_kind_of Tipi::Converter::CompositeConverter, doc.converter
      %w(paragraph).each do |node_name|
        selected = doc.converter.find_converter node_name
        assert_kind_of Tipi::Converter::TemplateConverter, selected
    ...
    test 'resolves templates from classloader when using JRuby', if: jruby? do
      ...
      * baz
      ...
      doc = Tipi.load input, template_dir: 'uri:classloader:templates', template_cache: false
      output = doc.convert
  ...
  context 'Custom converters' do
    test 'should not expose included method on Converter class' do
      refute_includes Tipi::Converter.methods, :included
      assert_includes Tipi::Converter.private_methods, :included
      refute_respond_to Tipi::Converter, :included
    end

    test 'should derive backend traits for the given backend' do
      expected = { basebackend: 'dita', filetype: 'dita', outfilesuffix: '.dita' }
      actual = Tipi::Converter.derive_backend_traits 'dita2'
    ...
    test 'should get converter from specified converter factory' do
      ...
      my_converter_class = Class.new Tipi::Converter::Base do
        def convert_document node
          'document'
        end
      end

      converter_factory = Tipi::Converter::CustomFactory.new 'html5' => my_converter_class
    ...
    test 'should allow converter to set htmlsyntax when basebackend is html' do
      ...
      converter = Tipi::Converter.create 'html5', htmlsyntax: 'xml'
    ...
    test 'should use converter registered for backend' do
      begin
        converters_before = Tipi::Converter.converters

        class CustomConverterB
          include Tipi::Converter
          register_for 'foobar'
          def initialize *args
            super
            basebackend 'text'
        ...
        input = 'content'
        assert_equal CustomConverterB, (Tipi::Converter.for 'foobar')
        converters = Tipi::Converter.converters
        ...
        Tipi::Converter.unregister_all
      end
    end

    test 'should be able to register converter using symbol' do
      begin
        converter = Class.new Tipi::Converter::Base do
          register_for :foobaz
          def initialize *args
            super
            basebackend 'text'
        ...
        assert_equal converter, (Tipi::Converter.for 'foobaz')
      ensure
        Tipi::Converter.unregister_all
      end
    end
    ...
    test 'should use basebackend to compute filetype and outfilesuffix' do
      begin
        assert_nil Tipi::Converter.for 'slides'

        class SlidesConverter < Tipi::Converter::Base
          register_for 'slides'
    ...
    test 'should be able to register converter from converter class itself' do
      begin
        assert_nil Tipi::Converter.for 'foobar'
        ...
        assert_equal AnotherCustomConverterB, (Tipi::Converter.for 'foobar')
      ensure
        Tipi::Converter.unregister_all
      end
    end
    ...
    test 'should wrap converter in composite converter with template converter if it declares that it supports templates' do
      begin
        class CustomConverterE < Tipi::Converter::Base
          register_for 'myhtml'
    ...
    test 'should map Factory.new to DefaultFactoryProxy constructor by default' do
      assert_equal (Tipi::Converter.for 'html5'), (Tipi::Converter::Factory.new.for 'html5')
    end

    test 'should map Factory.new to CustomFactory constructor if proxy keyword arg is false' do
      assert_nil (Tipi::Converter::Factory.new proxy_default: false).for 'html5'
    end
    ...
    test 'should default to catch all converter' do
      begin
        ...
        input = 'content'
        assert_equal CustomConverterF, (Tipi::Converter.for 'all')
        assert_equal CustomConverterF, (Tipi::Converter.for 'whatever')
        refute_equal CustomConverterF, (Tipi::Converter.for 'html5')
        converters = Tipi::Converter.converters
        assert_nil converters['*']
        assert_equal CustomConverterF, (Tipi::Converter.send :catch_all)
    ...
    test 'should use catch all converter from custom factory only if no other converter matches' do
      class FooConverter < Tipi::Converter::Base
      end

      class CatchAllConverter < Tipi::Converter::Base
      end

      factory = Tipi::Converter::CustomFactory.new 'foo' => FooConverter, '*' => CatchAllConverter
    ...
    test 'should prefer catch all converter from proxy over statically registered catch all converter' do
      begin
        class StaticCatchAllConverter < Tipi::Converter::Base
          register_for '*'
        end

        class LocalCatchAllConverter < Tipi::Converter::Base; end

        factory = Tipi::Converter::DefaultFactoryProxy.new '*' => LocalCatchAllConverter
    ...
    test 'should prefer converter in proxy with same name as provided converter' do
      class MyHtml5Converter < Tipi::Converter::Base; end
      factory = Tipi::Converter::DefaultFactoryProxy.new 'html5' => MyHtml5Converter
      assert_equal MyHtml5Converter, (factory.for 'html5')
    end

    test 'should allow nil to be registered as converter' do
      factory = Tipi::Converter::DefaultFactoryProxy.new 'html5' => nil
      assert_nil factory.for 'html5'
    end

    test 'should create a new custom factory when Converter::Factory.new is invoked' do
      class MyConverter < Tipi::Converter::Base; end
      converters = { 'mine' => MyConverter }
      factory = Tipi::Converter::Factory.new converters
      assert_kind_of Tipi::Converter::CustomFactory, factory
