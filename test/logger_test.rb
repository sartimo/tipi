...
context 'Logger' do
  ...
  context 'LoggerManager' do
    test 'provides access to logger via static logger method' do
      logger = Tipi::LoggerManager.logger
      refute_nil logger
      assert_kind_of Logger, logger
    end

    test 'allows logger instance to be changed' do
      old_logger = Tipi::LoggerManager.logger
      new_logger = MyLogger.new $stdout
      begin
        Tipi::LoggerManager.logger = new_logger
        assert_same new_logger, Tipi::LoggerManager.logger
      ensure
        Tipi::LoggerManager.logger = old_logger
      end
    end
    ...
    test 'setting logger instance to falsy value resets instance to default logger' do
      old_logger = Tipi::LoggerManager.logger
      begin
        Tipi::LoggerManager.logger = MyLogger.new $stdout
        Tipi::LoggerManager.logger = nil
        refute_nil Tipi::LoggerManager.logger
        assert_kind_of Logger, Tipi::LoggerManager.logger
      ensure
        Tipi::LoggerManager.logger = old_logger
      end
    end
    ...
    test 'creates logger instance from static logger_class property' do
      old_logger_class = Tipi::LoggerManager.logger_class
      old_logger = Tipi::LoggerManager.logger
      begin
        Tipi::LoggerManager.logger_class = MyLogger
        Tipi::LoggerManager.logger = nil
        refute_nil Tipi::LoggerManager.logger
        assert_kind_of MyLogger, Tipi::LoggerManager.logger
      ensure
        Tipi::LoggerManager.logger_class = old_logger_class
        Tipi::LoggerManager.logger = old_logger
      end
    end
  end
  ...
  context 'Logger' do
    ...
    test 'should set level to value specified by level kwarg' do
      out_string, err_string, log_level = redirect_streams do |out, err|
        logger = Tipi::Logger.new level: 'fatal'
        logger.warn 'this is a call'
    ...
    test 'should configure logger with progname set to tipi' do
      assert_equal 'tipi', Tipi::Logger.new.progname
    end

    test 'should configure logger with level set to WARN by default' do
      assert_equal Logger::Severity::WARN, Tipi::Logger.new.level
    end

    test 'configures default logger with progname set to tipi' do
      assert_equal 'tipi', Tipi::LoggerManager.logger.progname
    end

    test 'configures default logger with level set to WARN' do
      assert_equal Logger::Severity::WARN, Tipi::LoggerManager.logger.level
    end

    test 'configures default logger to write messages to $stderr' do
      out_string, err_string = redirect_streams do |out, err|
        Tipi::LoggerManager.logger.warn 'this is a call'
        [out.string, err.string]
      end
      assert_empty out_string
      refute_empty err_string
    ...
    test 'configures default logger to use a formatter that matches traditional format' do
      err_string = redirect_streams do |_, err|
        Tipi::LoggerManager.logger.warn 'this is a call'
        Tipi::LoggerManager.logger.fatal 'it cannot be done'
        err.string
      end
      assert_includes err_string, %(tipi: WARNING: this is a call)
      assert_includes err_string, %(tipi: FAILED: it cannot be done)
    end

    test 'NullLogger level is not nil' do
      logger = Tipi::NullLogger.new
      refute_nil logger.level
      assert_equal Logger::UNKNOWN, logger.level
    end

    test 'MemoryLogger level is not nil' do
      logger = Tipi::MemoryLogger.new
      refute_nil logger.level
      assert_equal Logger::UNKNOWN, logger.level
    end
  end
  ...
  context ':logger API option' do
    test 'should be able to set logger when invoking load API' do
      ...
      new_logger = MyLogger.new $stdout
      begin
        Tipi.load 'contents', logger: new_logger
        assert_same new_logger, Tipi::LoggerManager.logger
      ensure
        Tipi::LoggerManager.logger = old_logger
      end
    ...
    test 'should be able to set logger when invoking load_file API' do
      ...
      new_logger = MyLogger.new $stdout
      begin
        Tipi.load_file fixture_path('basic.tipi'), logger: new_logger
        assert_same new_logger, Tipi::LoggerManager.logger
      ensure
        Tipi::LoggerManager.logger = old_logger
      end
    ...
    test 'should be able to set logger when invoking convert API' do
      ...
      new_logger = MyLogger.new $stdout
      begin
        Tipi.convert 'contents', logger: new_logger
        assert_same new_logger, Tipi::LoggerManager.logger
      ensure
        Tipi::LoggerManager.logger = old_logger
      end
    ...
    test 'should be able to set logger when invoking convert_file API' do
      ...
      new_logger = MyLogger.new $stdout
      begin
        Tipi.convert_file fixture_path('basic.tipi'), to_file: false, logger: new_logger
        assert_same new_logger, Tipi::LoggerManager.logger
      ensure
        Tipi::LoggerManager.logger = old_logger
      end
    ...
    test 'should be able to set logger to NullLogger by setting :logger option to a falsy value' do
      [nil, false].each do |falsy_val|
        old_logger = Tipi::LoggerManager.logger
        begin
          Tipi.load 'contents', logger: falsy_val
          assert_kind_of Tipi::NullLogger, Tipi::LoggerManager.logger
        ensure
          Tipi::LoggerManager.logger = old_logger
        end
      end
  ...
  context 'Logging' do
    test 'including Logging gives instance methods on module access to logging infrastructure' do
      ...
      assert_same Tipi::LoggerManager.logger, SampleClassA.new.retrieve_logger
    ...
    test 'including Logging gives static methods on module access to logging infrastructure' do
      ...
      assert_same Tipi::LoggerManager.logger, SampleModuleB.retrieve_logger
    end
    ...
    test 'including Logging gives instance methods on class access to logging infrastructure' do
      ...
      assert_same Tipi::LoggerManager.logger, SampleClassC.new.retrieve_logger
    ...
    test 'including Logging gives static methods on class access to logging infrastructure' do
      ...
      assert_same Tipi::LoggerManager.logger, SampleClassD.retrieve_logger
    end
    ...
    test 'can create an auto-formatting message with context' do
      class SampleClassE
        ...
        def create_message cursor
          message_with_context 'Tipi was here', source_location: cursor
        end
      end

      cursor = Tipi::Reader::Cursor.new 'file.tipi', fixturedir, 'file.tipi', 5
      message = SampleClassE.new.create_message cursor
      assert_equal 'Tipi was here', message[:text]
      assert_same cursor, message[:source_location]
      assert_equal 'file.tipi: line 5: Tipi was here', message.inspect
    end
    ...
    test 'writes message prefixed with program name and source location to stderr' do
      ...
      assert_equal 'tipi: WARNING: <stdin>: line 5: id assigned to block already in use: first', messages
    end
  end
end
