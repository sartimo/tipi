...
TIPI_TEST_DIR = File.absolute_path __dir__
TIPI_LIB_DIR = ENV['TIPI_LIB_DIR'] || (File.join TIPI_TEST_DIR, '../lib')

require 'simplecov' if ENV['COVERAGE'] == 'true'

require File.join TIPI_LIB_DIR, 'tipi'
Dir.chdir Tipi::ROOT_DIR
...
class Minitest::Test
  ...
  def disk_root
    %(#{windows? ? (Tipi::ROOT_DIR.partition '/')[0] : ''}/)
  end

  def empty_document options = {}
    options[:parse] ? (Tipi::Document.new [], options).parse : (Tipi::Document.new [], options)
  end

  def empty_safe_document options = {}
    Tipi::Document.new [], (options.merge safe: :safe)
  end

  def sample_doc_path name
    unless (name = name.to_s).include? '.'
      %w(tipi tipi txt).each do |ext|
  ...
  def example_document name, opts = {}
    document_from_string (File.read (sample_doc_path name), mode: Tipi::FILE_READ_MODE), opts
  end
  ...
  def document_from_string src, opts = {}
    ...
    opts[:parse] ? (Tipi::Document.new src.lines, opts).parse : (Tipi::Document.new src.lines, opts)
  end
  ...
  def parse_header_metadata source, doc = nil
    reader = Tipi::Reader.new source.split Tipi::LF
    Tipi::Parser.parse_header_metadata reader, doc
  end
  ...
  def invoke_cli_with_filenames argv = [], filenames = [], &block
    ...
    invoker = Tipi::Cli::Invoker.new argv + filepaths
    invoker.invoke!(&block)
    invoker
  end

  def invoke_cli_to_buffer argv = [], filename = 'sample.tipi', &block
    invoke_cli argv, filename, [StringIO.new, StringIO.new], &block
  end

  def invoke_cli argv = [], filename = 'sample.tipi', buffers = nil, &block
    if filename.nil? || filename == '-' || (Pathname.new filename).absolute?
      filepath = filename
    else
      filepath = fixture_path filename
    end
    invoker = Tipi::Cli::Invoker.new argv + [filepath]
  ...
  def redirect_streams
    ...
    new_logger = (Tipi::LoggerManager.logger = Tipi::Logger.new $stderr)
  ...
  def using_memory_logger level = nil
    old_logger = Tipi::LoggerManager.logger
    memory_logger = Tipi::MemoryLogger.new
    memory_logger.level = level if level
    begin
      Tipi::LoggerManager.logger = memory_logger
      yield memory_logger
    ensure
      Tipi::LoggerManager.logger = old_logger
    end
  ...
  def in_verbose_mode
    begin
      old_logger_level, Tipi::LoggerManager.logger.level = Tipi::LoggerManager.logger.level, Logger::Severity::DEBUG
      yield
    ensure
      Tipi::LoggerManager.logger.level = old_logger_level
    end
  end

  def tipi_cmd ruby_args = nil
    [Gem.ruby, *ruby_args, (File.join bindir, 'tipi')]
  end
  ...
  def using_test_webserver host = resolve_localhost, port = 9876
    ...
    server_thread = Thread.start do
      ...
      while (session = server.accept)
        ...
        if resource == '/name/tipi'
          session.print %(HTTP/1.1 200 OK\r\nContent-Type: application/json\r\n\r\n)
          session.print %({"name": "tipi"}\n)
        elsif File.file? resource_file = (File.join base_dir, resource)
          if (ext = File.extname(resource_file)[1..-1])
            mimetype = ext == 'tipi' ? 'text/plain' : %(image/#{ext})
          else
            mimetype = 'text/plain'
          end
          session.print %(HTTP/1.1 200 OK\r\nContent-Type: #{mimetype}\r\n\r\n)
          File.open resource_file, Tipi::FILE_READ_MODE do |fd|
