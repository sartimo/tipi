...
context 'Syntax Highlighter' do
  test 'should set syntax_highlighter property on document if source highlighter is set and basebackend is html' do
    ...
    assert_kind_of Tipi::SyntaxHighlighter, doc.syntax_highlighter
  end
  ...
  test 'should not allow document to enable syntax highlighter if safe mode is at least SERVER' do
    ...
    doc = document_from_string input, safe: Tipi::SafeMode::SERVER, parse: true
  ...
  test 'should not invoke highlight method on syntax highlighter if highlight? is false' do
    Class.new Tipi::SyntaxHighlighter::Base do
  ...
  test 'should be able to register syntax highlighter from syntax highlighter class itself' do
    syntax_highlighter = Class.new Tipi::SyntaxHighlighter::Base do
      def format node, language, opts
        %(<pre class="highlight"><code class="language-#{language}" data-lang="#{language}">#{node.content}</code></pre>)
      end
    ...
    assert_equal syntax_highlighter, (Tipi::SyntaxHighlighter.for 'foobar')
  end
  ...
  test 'should be able to register syntax highlighter using symbol' do
    ...
    assert_equal syntax_highlighter, (Tipi::SyntaxHighlighter.for 'foobaz')
  end
  ...
  test 'should highlight source inside Tipi table cell if source-highlighter attribute is set' do
    input = <<~'EOS'
    :source-highlighter: coderay
  ...
  test 'should set starting line number in DocBook output if linenums option is enabled and start attribute is set' do
    ...
    output = convert_string_to_embedded input, backend: :docbook, safe: Tipi::SafeMode::SAFE
  ...
  test 'should read source language from source-language document attribute if not specified on source block' do
    ...
    ----
    EOS
    output = convert_string_to_embedded input, safe: Tipi::SafeMode::SAFE, linkcss_default: true
  ...
  context 'CodeRay' do
    test 'should highlight source if source-highlighter attribute is set' do
      ...
      ----
      EOS
      output = convert_string input, safe: Tipi::SafeMode::SAFE, linkcss_default: true
      assert_xpath '//pre[@class="CodeRay highlight"]/code[@data-lang="ruby"]//span[@class = "constant"][text() = "CodeRay"]', output, 1
    ...
    test 'should not fail if source language is invalid' do
      ...
      ----
      EOS
      output = convert_string_to_embedded input, safe: Tipi::SafeMode::SAFE
    ...
    test 'should preserve space before callout on final line' do
      ...
      inputs.each do |input|
        output = convert_string_to_embedded input, safe: Tipi::SafeMode::SAFE, attributes: { 'source-highlighter' => 'coderay' }
    ...
    test 'should preserve passthrough placeholders when highlighting source using coderay' do
      ...
      refute_includes output, Tipi::Substitutors::PASS_START
    end
    ...
    test 'should link to CodeRay stylesheet if source-highlighter is coderay and linkcss is set' do
      ...
      ----
      EOS
      output = convert_string input, safe: Tipi::SafeMode::SAFE, attributes: { 'linkcss' => '' }
      assert_xpath '//pre[@class="CodeRay highlight"]/code[@data-lang="ruby"]//span[@class = "constant"][text() = "CodeRay"]', output, 1
      link_node = xmlnodes_at_xpath '//link[@rel="stylesheet"][@href="./coderay-tipi.css"]', output, 1
    ...
    test 'should read stylesheet' do
      css = (Tipi::SyntaxHighlighter.for 'coderay').read_stylesheet
      refute_nil css
  ...
  context 'Rouge' do
    ...
    test 'should gracefully fallback to default style if specified style not recognized' do
      ...
      expected_css = (Tipi::SyntaxHighlighter.for 'rouge').read_stylesheet 'github'
    ...
    test 'should read stylesheet for specified style' do
      css = (Tipi::SyntaxHighlighter.for 'rouge').read_stylesheet 'monokai'
      refute_nil css
      assert_includes css, 'pre.rouge {'
      assert_includes css, 'background-color: #49483e;'
    end

    test 'should not fail to load rouge if the Tipi module is included into the global namespace' do
      result = run_command(tipi_cmd, '-r', (fixture_path 'include-tipi.rb'), '-s', '-o', '-', '-a', 'source-highlighter=rouge', (fixture_path 'source-block.tipi'), use_bundler: true) {|out| out.read }
