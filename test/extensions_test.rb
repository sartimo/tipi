...
class ExtensionsInitTest < Minitest::Test
  def test_autoload
    ...
    Tipi::Extensions.unregister_all
  end
  self
end.new(nil).test_autoload

class SamplePreprocessor < Tipi::Extensions::Preprocessor
  def process doc, reader
    nil
  end
end

class SampleIncludeProcessor < Tipi::Extensions::IncludeProcessor
end

class SampleDocinfoProcessor < Tipi::Extensions::DocinfoProcessor
end

# NOTE intentionally using the deprecated name
class SampleTreeprocessor < Tipi::Extensions::Treeprocessor
...
class SamplePostprocessor < Tipi::Extensions::Postprocessor
end

class SampleBlock < Tipi::Extensions::BlockProcessor
end

class SampleBlockMacro < Tipi::Extensions::BlockMacroProcessor
end

class SampleInlineMacro < Tipi::Extensions::InlineMacroProcessor
end

class ScrubHeaderPreprocessor < Tipi::Extensions::Preprocessor
  def process doc, reader
    lines = reader.lines
    skipped = []
...
class BoilerplateTextIncludeProcessor < Tipi::Extensions::IncludeProcessor
  def handles? target
    target.end_with? '.txt'
  end
...
class ReplaceAuthorTreeProcessor < Tipi::Extensions::TreeProcessor
  def process document
...
class ReplaceTreeTreeProcessor < Tipi::Extensions::TreeProcessor
  def process document
    if document.doctitle == 'Original Document'
      Tipi.load %(== Replacement Document\nReplacement Author\n\ncontent)
    else
      document
    end
  end
...
class SelfSigningTreeProcessor < Tipi::Extensions::TreeProcessor
  def process document
...
class StripAttributesPostprocessor < Tipi::Extensions::Postprocessor
  def process document, output
    output.gsub %r/<(\w+).*?>/m, '<\\1>'
  end
end

class UppercaseBlock < Tipi::Extensions::BlockProcessor
  use_dsl
  named :yell
...
class SnippetMacro < Tipi::Extensions::BlockMacroProcessor
  def process parent, target, attributes
    create_pass_block parent, %(<script src="http://example.com/#{target}.js?_mode=#{attributes['mode']}"></script>), {}, content_model: :raw
  end
end

class LegacyPosAttrsBlockMacro < Tipi::Extensions::BlockMacroProcessor
...
class TemperatureMacro < Tipi::Extensions::InlineMacroProcessor
  use_dsl
  named :degrees
...
class MetaRobotsDocinfoProcessor < Tipi::Extensions::DocinfoProcessor
  def process document
    '<meta name="robots" content="index,follow">'
  end
end

class MetaAppDocinfoProcessor < Tipi::Extensions::DocinfoProcessor
  use_dsl
  at_location :head

  def process document
    '<meta name="application-name" content="Tipi App">'
  end
end

class SampleExtensionGroup < Tipi::Extensions::Group
...
context 'Extensions' do
  context 'Register' do
    test 'should not activate registry if no extension groups are registered' do
      assert defined? Tipi::Extensions
      doc = empty_document
    ...
    test 'should register extension group class' do
      begin
        Tipi::Extensions.register :sample, SampleExtensionGroup
        refute_nil Tipi::Extensions.groups
        assert_equal 1, Tipi::Extensions.groups.size
        assert_equal SampleExtensionGroup, Tipi::Extensions.groups[:sample]
      ensure
        Tipi::Extensions.unregister_all
      end
    end
    ...
    test 'should register extension group from instance' do
      begin
        Tipi::Extensions.register :sample, SampleExtensionGroup.new
        refute_nil Tipi::Extensions.groups
        assert_equal 1, Tipi::Extensions.groups.size
        assert_kind_of SampleExtensionGroup, Tipi::Extensions.groups[:sample]
      ensure
        Tipi::Extensions.unregister_all
      end
    end
    ...
    test 'should register extension block' do
      begin
        Tipi::Extensions.register :sample do
          # this space intentionally left blank
        end
        refute_nil Tipi::Extensions.groups
        assert_equal 1, Tipi::Extensions.groups.size
        assert_kind_of Proc, Tipi::Extensions.groups[:sample]
      ensure
        Tipi::Extensions.unregister_all
      end
    end
    ...
    test 'should unregister extension group by symbol name' do
      begin
        ...
        refute_nil Tipi::Extensions.groups
        assert_equal 1, Tipi::Extensions.groups.size
        Tipi::Extensions.unregister :sample
        assert_equal 0, Tipi::Extensions.groups.size
      ensure
        Tipi::Extensions.unregister_all
      end
    end
    ...
    test 'should unregister multiple extension groups by name' do
      begin
        Tipi::Extensions.register :sample1, SampleExtensionGroup
        Tipi::Extensions.register :sample2, SampleExtensionGroup
        refute_nil Tipi::Extensions.groups
        assert_equal 2, Tipi::Extensions.groups.size
        Tipi::Extensions.unregister :sample1, :sample2
        assert_equal 0, Tipi::Extensions.groups.size
      ensure
        Tipi::Extensions.unregister_all
      end
    ...
    test 'should not fail to unregister extension group if extension groups are not initialized' do
      Tipi::Extensions.remove_instance_variable :@groups
      Tipi::Extensions.unregister :sample
      assert_equal 0, Tipi::Extensions.groups.size
    end
    ...
    test 'should allow standalone registry to be created but not registered' do
      registry = Tipi::Extensions.create 'sample' do
        block do
          named :whisper
          on_context :paragraph
          parse_content_as :simple
      ...
      assert_instance_of Tipi::Extensions::Registry, registry
  ...
  context 'Activate' do
    test 'should call activate on extension group class' do
      begin
        ...
        Tipi::Extensions.register :sample, SampleExtensionGroup
        registry = Tipi::Extensions::Registry.new
    ...
    test 'should create registry in Document if extensions are loaded' do
      begin
        ...
        assert_kind_of Tipi::Extensions::Registry, doc.extensions
      ensure
        Tipi::Extensions.unregister_all
      end
    end
  ...
  context 'Instantiate' do
    test 'should instantiate preprocessors' do
      registry = Tipi::Extensions::Registry.new
      registry.preprocessor SamplePreprocessor
      registry.activate Tipi::Document.new
      assert registry.preprocessors?
      extensions = registry.preprocessors
      assert_equal 1, extensions.size
      assert_kind_of Tipi::Extensions::ProcessorExtension, extensions.first
    ...
    test 'should instantiate block processor' do
      ...
      assert_kind_of Tipi::Extensions::ProcessorExtension, extension
  ...
  context 'Integration' do
    test 'does not crash when querying for extensions if none are registered' do
      registry = Tipi::Extensions.create
    ...
    test 'can provide extension registry as an option' do
      registry = Tipi::Extensions.create do
    ...
    test 'should invoke include processor if it offers to handle include directive' do
      input = <<~'EOS'
      include::skip-me.tipi[]
      line after skip

      include::include-file.tipi[]

      include::fixtures/grandchild-include.tipi[]
      ...
      lines << reader.read_line
      lines << reader.read_line
      assert_equal 'found include target \'include-file.tipi\' at line 4', lines.last
      assert_equal 'include-file.tipi: line 2', reader.line_info
      lines << reader.read_line while reader.has_more_lines?
      source = lines * Tipi::LF
      assert_match(/^found include target 'include-file.tipi' at line 4$/, source)
    ...
    test 'should set source_location on document before invoking tree processors' do
      begin
        ...
        sample_doc = fixture_path 'sample.tipi'
        doc = Tipi.load_file sample_doc, sourcemap: true
        assert_includes doc.convert, 'file: sample.tipi, lineno: 1'
      ensure
        Tipi::Extensions.unregister_all
      end
    end
    ...
    test 'should invoke processor for custom block in an Tipi table cell' do
      input = <<~'EOS'
      |===
      a|
      [yell]
    ...
    test 'should invoke processor for custom block macro in an Tipi table cell' do
      input = <<~'EOS'
      |===
      a|message::hi[]
      |===
    ...
    test 'should add multiple docinfo to document' do
      ...
      begin
        ...
        assert_equal %(<meta name="robots" content="index,follow">\n<meta name="application-name" content="Tipi App">), doc.docinfo
    ...
    test 'should append docinfo to document' do
      begin
        ...
        output = Tipi.convert_file sample_input_path, to_file: false, standalone: true, safe: Tipi::SafeMode::SERVER, attributes: { 'docinfo' => '' }
        refute_empty output
    ...
    test 'should return extension instance after registering' do
      begin
        ...
        empty_document
        exts.each do |ext|
          assert_kind_of Tipi::Extensions::ProcessorExtension, ext
        end
      ensure
        Tipi::Extensions.unregister_all
      end
    ...
    test 'should raise exception if document processor extension does not provide process method' do
      extension_registry = Tipi::Extensions.create do
