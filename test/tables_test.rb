...
context 'Tables' do
  context 'PSV' do
    ...
    test 'should format first cell as Tipi if there is no implicit header row and column has a style' do
      input = <<~'EOS'
      [cols="1a,1"]
      |===
      | * list
    ...
    test 'should interpret leading indent if first cell is Tipi and there is no implicit header row' do
      input = <<~'EOS'
      [cols="1a,1"]
      |===
      |
    ...
    test 'should format first cell as Tipi if there is no implicit header row and cell has a style' do
      input = <<~'EOS'
      |===
      a| * list
      | normal
    ...
    test 'no implicit header row if Tipi cell in first line spans multiple lines' do
      input = <<~'EOS'
      [cols=2*]
      |===
      a|contains Tipi content

      * a
      * b
      * c
      a|contains no Tipi content
    ...
    test 'Tipi table cell should be wrapped in div with class "content"' do
      input = <<~'EOS'
      |===
      a|Tipi table cell
      |===
    ...
    test 'doctype can be set in Tipi table cell' do
      input = <<~'EOS'
      |===
      a|
      :doctype: inline
    ...
    test 'should reset doctype to default in Tipi table cell' do
      input = <<~'EOS'
      = Book Title
      :doctype: book
    ...
    test 'should update doctype-related attributes in Tipi table cell when doctype is set' do
      input = <<~'EOS'
      = Document Title
      :doctype: article
    ...
    test 'should not allow Tipi table cell to set a document attribute that was hard set by the API' do
      input = <<~'EOS'
      |===
      a|
      :icons:
    ...
    test 'should not allow Tipi table cell to set a document attribute that was hard unset by the API' do
      input = <<~'EOS'
      |===
      a|
      :icons: font
    ...
    test 'should keep attribute unset in Tipi table cell if unset in parent document' do
      input = <<~'EOS'
      :!sectids:
      :!table-caption:
    ...
    test 'should allow attribute unset in parent document to be set in Tipi table cell' do
      input = <<~'EOS'
      :!sectids:
    ...
    test 'should not allow locked attribute unset in parent document to be set in Tipi table cell' do
      input = <<~'EOS'
      == No ID
    ...
    test 'showtitle can be enabled in Tipi table cell if unset in parent document' do
    ...
    test 'showtitle can be enabled in Tipi table cell if unset by API' do
      %w(showtitle notitle).each do |name|
        input = <<~EOS
        = Document Title
    ...
    test 'showtitle can be disabled in Tipi table cell if set in parent document' do
      %w(showtitle notitle).each do |name|
        input = <<~EOS
        = Document Title
        :#{name == 'showtitle' ? '' : '!'}#{name}:
    ...
    test 'showtitle can be disabled in Tipi table cell if set by API' do
      %w(showtitle notitle).each do |name|
        input = <<~EOS
        = Document Title
    ...
    test 'Tipi content' do
      ...
      Where `<filename>` is the file name (sans extension) of the Tipi
    ...
    test 'should preserve leading indentation in contents of Tipi table cell if contents starts with newline' do
      input = <<~'EOS'
      |===
      a|
       $ command
    ...
    test 'preprocessor directive on first line of an Tipi table cell should be processed' do
      input = <<~'EOS'
      |===
      a|include::fixtures/include-file.tipi[]
      |===
      EOS
    ...
    test 'cross reference link in an Tipi table cell should resolve to reference in main document' do
      input = <<~'EOS'
      == Some
    ...
    test 'footnotes should not be shared between an Tipi table cell and the main document' do
      input = <<~'EOS'
      |===
      a|Tipi footnote:[A lightweight markup language.]
      |===
      EOS
    ...
    test 'callout numbers should be globally unique, including Tipi table cells' do
      input = <<~'EOS'
      = Document Title
    ...
    test 'compat mode can be activated in Tipi table cell' do
      input = <<~'EOS'
      |===
      a|
      :compat-mode:
    ...
    test 'compat mode in Tipi table cell inherits from parent document' do
      input = <<~'EOS'
      :compat-mode:
    ...
    test 'compat mode in Tipi table cell can be unset if set in parent document' do
      input = <<~'EOS'
      :compat-mode:
    ...
    test 'Tipi table cell should inherit to_dir option from parent document' do
    ...
    test 'Tipi table cell should not inherit toc setting from parent document' do
      input = <<~'EOS'
      = Document Title
      :toc:
    ...
    test 'should be able to enable toc in an Tipi table cell' do
      input = <<~'EOS'
      = Document Title
    ...
    test 'should be able to enable toc in an Tipi table cell even if hard unset by API' do
      input = <<~'EOS'
      = Document Title
    ...
    test 'should be able to enable toc in both outer document and in an Tipi table cell' do
      input = <<~'EOS'
      = Document Title
      :toc:
    ...
    test 'document in an Tipi table cell should not see doctitle of parent' do
      input = <<~'EOS'
      = Document Title
    ...
    test 'should show correct line number in warning about unterminated block inside Tipi table cell' do
      input = <<~'EOS'
      outside
    ...
    test 'custom separator for an Tipi table cell' do
      input = <<~'EOS'
      [cols=2,separator=!]
      |===
      !Pipe output to vim
  ...
  context 'CSV' do
    ...
    test 'should not drop trailing empty cell in TSV data when loaded from an include file' do
      ...
      |===
      EOS
      output = convert_string_to_embedded input, safe: :safe, base_dir: TIPI_TEST_DIR
      assert_css 'table > tbody > tr', output, 3
      assert_css 'table > tbody > tr:nth-child(1) > td', output, 3
      assert_css 'table > tbody > tr:nth-child(2) > td', output, 3
      assert_css 'table > tbody > tr:nth-child(3) > td', output, 3
    ...
    test 'cell formatted with Tipi style' do
      input = <<~'EOS'
    ...
    test 'should strip whitespace around contents of Tipi cell' do
      input = <<~'EOS'
      [cols="1,1,1a",separator=;]
      ,===
      element;description;example
