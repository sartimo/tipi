...
context 'Links' do
  test 'qualified url inline with text' do
    assert_xpath '//a[@href="http://tipi.org"][@class="bare"][text() = "http://tipi.org"]', convert_string('The Tipi project is located at http://tipi.org.')
  end

  test 'qualified url with role inline with text' do
    assert_xpath '//a[@href="http://tipi.org"][@class="bare project"][text() = "http://tipi.org"]', convert_string('The Tipi project is located at http://tipi.org[role=project].')
  end

  test 'qualified http url inline with hide-uri-scheme set' do
    assert_xpath '//a[@href="http://tipi.org"][@class="bare"][text() = "tipi.org"]', convert_string('The Tipi project is located at http://tipi.org.', attributes: { 'hide-uri-scheme' => '' })
  end
  ...
  test 'qualified url with label' do
    assert_xpath '//a[@href="http://tipi.org"][text() = "Tipi"]', convert_string("We're parsing http://tipi.org[Tipi] markup")
  end

  test 'qualified url with label containing escaped right square bracket' do
    assert_xpath '//a[@href="http://tipi.org"][text() = "[Ascii]Doc"]', convert_string("We're parsing http://tipi.org[[Ascii\\]Doc] markup")
  end
  ...
  test 'qualified url with label using link macro' do
    assert_xpath '//a[@href="http://tipi.org"][text() = "Tipi"]', convert_string("We're parsing link:http://tipi.org[Tipi] markup")
  end

  test 'qualified url with role using link macro' do
    assert_xpath '//a[@href="http://tipi.org"][@class="bare project"][text() = "http://tipi.org"]', convert_string("We're parsing link:http://tipi.org[role=project] markup")
  end

  test 'qualified url using macro syntax with multi-line label inline with text' do
    assert_xpath %{//a[@href='http://tipi.org'][text() = 'Tipi\nmarkup']}, convert_string("We're parsing link:http://tipi.org[Tipi\nmarkup]")
  end
  ...
  test 'qualified url surrounded by angled brackets' do
    assert_xpath '//a[@href="http://tipi.org"][text()="http://tipi.org"]', convert_string('<http://tipi.org> is the project page for Tipi.'), 1
  end
  ...
  test 'qualified url with trailing period' do
    result = convert_string_to_embedded 'The homepage for Tipi is https://tipi.org.'
    assert_xpath '//a[@href="https://tipi.org"][text()="https://tipi.org"]', result, 1
    assert_xpath '//a[@href="https://tipi.org"][text()="https://tipi.org"]/following-sibling::text()[starts-with(.,".")]', result, 1
  end

  test 'qualified url with trailing explanation point' do
    result = convert_string_to_embedded 'Check out https://tipi.org!'
    assert_xpath '//a[@href="https://tipi.org"][text()="https://tipi.org"]', result, 1
    assert_xpath '//a[@href="https://tipi.org"][text()="https://tipi.org"]/following-sibling::text()[starts-with(.,"!")]', result, 1
  end

  test 'qualified url with trailing question mark' do
    result = convert_string_to_embedded 'Is the homepage for Tipi https://tipi.org?'
    assert_xpath '//a[@href="https://tipi.org"][text()="https://tipi.org"]', result, 1
    assert_xpath '//a[@href="https://tipi.org"][text()="https://tipi.org"]/following-sibling::text()[starts-with(.,"?")]', result, 1
  end

  test 'qualified url with trailing round bracket' do
    result = convert_string_to_embedded 'Tipi is a Ruby-based Tipi processor (see https://tipi.org)'
    assert_xpath '//a[@href="https://tipi.org"][text()="https://tipi.org"]', result, 1
    assert_xpath '//a[@href="https://tipi.org"][text()="https://tipi.org"]/following-sibling::text()[starts-with(.,")")]', result, 1
  end
  ...
  test 'qualified url with trailing semi-colon' do
    result = convert_string_to_embedded 'https://tipi.org; where text gets parsed'
    assert_xpath '//a[@href="https://tipi.org"][text()="https://tipi.org"]', result, 1
    assert_xpath '//a[@href="https://tipi.org"][text()="https://tipi.org"]/following-sibling::text()[starts-with(.,";")]', result, 1
  end
  ...
  test 'qualified url with trailing round bracket followed by colon' do
    result = convert_string_to_embedded '(from https://tipi.org): where text gets parsed'
    assert_xpath '//a[@href="https://tipi.org"][text()="https://tipi.org"]', result, 1
    assert_xpath '//a[@href="https://tipi.org"][text()="https://tipi.org"]/following-sibling::text()[starts-with(., "):")]', result, 1
  end
  ...
  test 'qualified url adjacent to text in square brackets' do
    assert_xpath '//a[@href="http://tipi.org"][text()="Tipi"]', convert_string(']http://tipi.org[Tipi] project page.'), 1
  end
  ...
  test 'qualified url following no-break space' do
    assert_xpath '//a[@href="http://tipi.org"][text()="Tipi"]', convert_string(%(#{[0xa0].pack 'U1'}http://tipi.org[Tipi] project page.)), 1
  end
  ...
  test 'should convert qualified url as macro enclosed in double quotes' do
    output = convert_string_to_embedded '"https://tipi.org[]"'
    assert_include '"<a href="https://tipi.org" class="bare">https://tipi.org</a>"', output
  end
  ...
  test 'qualified url using invalid link macro should not create link' do
    assert_xpath '//a', convert_string('link:http://tipi.org is the project page for Tipi.'), 0
  end

  test 'escaped inline qualified url should not create link' do
    assert_xpath '//a', convert_string('\http://tipi.org is the project page for Tipi.'), 0
  end

  test 'escaped inline qualified url as macro should not create link' do
    output = convert_string '\http://tipi.org[tipi.org] is the project page for Tipi.'
    assert_xpath '//a', output, 0
    assert_xpath '//p[starts-with(text(), "http://tipi.org[tipi.org]")]', output, 1
  end
  ...
  test 'escaped inline qualified url using macro syntax should not create link' do
    assert_xpath '//a', convert_string('\http://tipi.org[Tipi] is the key to good docs.'), 0
  end

  test 'inline qualified url followed by a newline should not include newline in link' do
    assert_xpath '//a[@href="https://github.com/tipi"]', convert_string("The source code for Tipi can be found at https://github.com/tipi\nwhich is a GitHub organization."), 1
  end

  test 'qualified url divided by newline using macro syntax should not create link' do
    assert_xpath '//a', convert_string("The source code for Tipi can be found at link:https://github.com/tipi\n[]which is a GitHub organization."), 0
  end

  test 'qualified url containing whitespace using macro syntax should not create link' do
    assert_xpath '//a', convert_string('I often need to refer to the chapter on link:http://tipi.org?q=attribute references[Attribute References].'), 0
  end

  test 'qualified url containing an encoded space using macro syntax should create a link' do
    assert_xpath '//a', convert_string('I often need to refer to the chapter on link:http://tipi.org?q=attribute%20references[Attribute References].'), 1
  end

  test 'inline quoted qualified url should not consume surrounding angled brackets' do
    assert_xpath '//a[@href="https://github.com/tipi"]', convert_string('Tipi GitHub organization: <**https://github.com/tipi**>'), 1
  end
  ...
  test 'inline ref' do
    ...
    variations.each do |anchor|
      ...
      output = doc.convert
      assert_kind_of Tipi::Inline, doc.catalog[:refs]['tigers']
  ...
  test 'xref using angled bracket syntax' do
    ...
    doc.register :refs, ['tigers', (Tipi::Inline.new doc, :anchor, '[tigers]', type: :ref, target: 'tigers'), '[tigers]']
  ...
  test 'inter-document xref shorthand syntax should assume Tipi extension if Tipi extension not present' do
    {
  ...
  test 'xref macro with explicit inter-document target should assume implicit Tipi file extension if no file extension is present' do
    {
      'using-.net-web-services#' => 'Using .NET web services',
      'tipi.1#' => 'Tipi Manual',
    }.each do |target, text|
  ...
  test 'inter-document xref should only remove the file extension part if the path contains a period elsewhere' do
    result = convert_string_to_embedded '<<using-.net-web-services.tipi#,Using .NET web services>>'
  ...
  test 'should not interpret double underscore in target of xref macro if sequence is preceded by a backslash' do
    result = convert_string_to_embedded 'xref:doc\__with_double__underscore.tipi[text]'
  ...
  test 'should not interpret double underscore in target of xref shorthand if sequence is preceded by a backslash' do
    result = convert_string_to_embedded '<<doc\__with_double__underscore.tipi#,text>>'
    assert_xpath '//a[@href="doc__with_double__underscore.html"][text() = "text"]', result, 1
  end
  ...
  test 'xref using angled bracket syntax with path and extension' do
    using_memory_logger do |logger|
      doc = document_from_string '<<tigers.tipi>>', standalone: false
      assert_xpath '//a[@href="#tigers.tipi"][text() = "[tigers.tipi]"]', doc.convert, 1
      assert_empty logger
    end
  end
  ...
  test 'xref using angled bracket syntax with path and extension with fragment' do
    doc = document_from_string '<<tigers.tipi#id>>', standalone: false
  ...
  test 'xref using macro syntax with path and extension in compat mode' do
    using_memory_logger do |logger|
      doc = document_from_string 'xref:tigers.tipi[]', standalone: false, attributes: { 'compat-mode' => '' }
      assert_xpath '//a[@href="#tigers.tipi"][text() = "[tigers.tipi]"]', doc.convert, 1
      assert_empty logger
    end
  end
  ...
  test 'xref using macro syntax with path and extension' do
    doc = document_from_string 'xref:tigers.tipi[]', standalone: false
    assert_xpath '//a[@href="tigers.html"][text() = "tigers.html"]', doc.convert, 1
  end
  ...
  test 'xref with target that begins with attribute reference in title' do
    ['<<{lessonsdir}/lesson-1#,Lesson 1>>', 'xref:{lessonsdir}/lesson-1.tipi[Lesson 1]'].each do |xref|
      input = <<~EOS
      :lessonsdir: lessons
  ...
  test 'should produce an internal anchor from an inter-document xref to file included into current file' do
    ...
    Read <<other-chapters.tipi#ch2>> to find out what happens next!

    include::other-chapters.tipi[]
    EOS
  ...
  test 'should produce an internal anchor from an inter-document xref to file included entirely into current file using tags' do
    ...
    include::other-chapters.tipi[tags=**]
    EOS
  ...
  test 'should not produce an internal anchor for inter-document xref to file partially included into current file' do
    ...
    Read <<other-chapters.tipi#ch2,the next chapter>> to find out what happens next!

    include::other-chapters.tipi[tags=ch2]
    EOS
  ...
  test 'should produce an internal anchor for inter-document xref to file included fully and partially' do
    ...
    include::other-chapters.tipi[tag=ch2-noid]
    EOS
  ...
  test 'should use doctitle of root document as fallback link text for inter-document xref in Tipi table cell that resolves to current doc' do
    input = <<~'EOS'
    = Document Title

    |===
    a|See xref:test.tipi[]
    |===
    EOS
    output = convert_string_to_embedded input, attributes: { 'docname' => 'test' }
  ...
  test 'should produce an internal anchor for inter-document xref to file outside of base directory' do
    ...
    See <<../section-a.tipi#section-a>>.

    include::../section-a.tipi[]
    EOS
