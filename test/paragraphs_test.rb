...
context 'Paragraphs' do
  ...
  context 'special' do
    test 'note multiline syntax' do
      Tipi::ADMONITION_STYLES.each do |style|
    ...
    context 'Inline doctype' do
      test 'should only format and output text in first paragraph when doctype is inline' do
        input = "http://tipi.org[Tipi] is a _lightweight_ markup language...\n\nignored"
        output = convert_string input, doctype: 'inline'
        assert_equal '<a href="http://tipi.org">Tipi</a> is a <em>lightweight</em> markup language&#8230;&#8203;', output
      end
