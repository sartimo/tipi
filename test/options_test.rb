...
context 'Options' do
  ...
  test 'should print usage and return error code 0 when help flag is unknown' do
    exitval, output = redirect_streams do |out, _|
      [Tipi::Cli::Options.parse!(%w(-h unknown)), out.string]
    end
    assert_equal 0, exitval
  ...
  test 'should dump man page and return error code 0 when help topic is manpage' do
    exitval, output = redirect_streams do |out, _|
      [Tipi::Cli::Options.parse!(%w(-h manpage)), out.string]
    end
    assert_equal 0, exitval
    assert_includes output, 'Manual: Tipi Manual'
    assert_includes output, '.TH "TIPI"'
  end

  test 'should an overview of the Tipi syntax and return error code 0 when help topic is syntax' do
    exitval, output = redirect_streams do |out, _|
      [Tipi::Cli::Options.parse!(%w(-h syntax)), out.string]
    end
    assert_equal 0, exitval
    assert_includes output, '= Tipi Syntax'
    assert_includes output, '== Text Formatting'
  end

  test 'should print message and return error code 1 when manpage is not found' do
    old_manpage_path = ENV['TIPI_MANPAGE_PATH']
    begin
      ENV['TIPI_MANPAGE_PATH'] = (manpage_path = fixture_path 'no-such-file.1')
      redirect_streams do |_, stderr|
        exitval = Tipi::Cli::Options.parse! %w(-h manpage)
        assert_equal 1, exitval
        assert_equal %(tipi: FAILED: manual page not found: #{manpage_path}), stderr.string.chomp
      end
    ensure
      if old_manpage_path
        ENV['TIPI_MANPAGE_PATH'] = old_manpage_path
      else
        ENV.delete 'TIPI_MANPAGE_PATH'
      end
    end
  end
  ...
  test 'should return error code 1 when invalid option present' do
    redirect_streams do |_, stderr|
      exitval = Tipi::Cli::Options.parse! %w(--foobar)
      assert_equal 1, exitval
      assert_equal 'tipi: invalid option: --foobar', stderr.string.chomp
    end
  end

  test 'should return error code 1 when option has invalid argument' do
    redirect_streams do |_, stderr|
      exitval = Tipi::Cli::Options.parse! %w(-d chapter input.ad) # had to change for #320
      assert_equal 1, exitval
      assert_equal 'tipi: invalid argument: -d chapter', stderr.string.chomp
    end
  end

  test 'should return error code 1 when option is missing required argument' do
    redirect_streams do |_, stderr|
      exitval = Tipi::Cli::Options.parse! %w(-b)
      assert_equal 1, exitval
      assert_equal 'tipi: option missing argument: -b', stderr.string.chomp
    end
  end

  test 'should emit warning when unparsed options remain' do
    redirect_streams do |_, stderr|
      options = Tipi::Cli::Options.parse! %w(-b docbook - -)
      assert_kind_of Hash, options
      assert_match(/tipi: WARNING: extra arguments .*/, stderr.string.chomp)
    end
  end

  test 'basic argument assignment' do
    options = Tipi::Cli::Options.parse! %w(-w -v -e -d book test/fixtures/sample.tipi)
    ...
    assert_equal 'test/fixtures/sample.tipi', options[:input_files][0]
  end

  test 'supports legacy option for no header footer' do
    options = Tipi::Cli::Options.parse! %w(-s test/fixtures/sample.tipi)
  ...
  test 'standard attribute assignment' do
    options = Tipi::Cli::Options.parse! %w(-a docinfosubs=attributes,replacements -a icons test/fixtures/sample.tipi)
  ...
  test 'multiple attribute arguments' do
    options = Tipi::Cli::Options.parse! %w(-a imagesdir=images -a icons test/fixtures/sample.tipi)
  ...
  test 'should only split attribute key/value pairs on first equal sign' do
    options = Tipi::Cli::Options.parse! %w(-a name=value=value test/fixtures/sample.tipi)

    assert_equal 'value=value', options[:attributes]['name']
  end

  test 'should not fail if value of attribute option is empty' do
    options = Tipi::Cli::Options.parse! ['-a', '', 'test/fixtures/sample.tipi']
  ...
  test 'should gracefully force encoding to UTF-8 if encoding on string is mislabeled' do
    ...
    options = Tipi::Cli::Options.parse! args
  ...
  test 'should allow safe mode to be specified' do
    options = Tipi::Cli::Options.parse! %w(-S safe test/fixtures/sample.tipi)
    assert_equal Tipi::SafeMode::SAFE, options[:safe]
  end

  test 'should allow any backend to be specified' do
    options = Tipi::Cli::Options.parse! %w(-b my_custom_backend test/fixtures/sample.tipi)

    assert_equal 'my_custom_backend', options[:attributes]['backend']
  end

  test 'article doctype assignment' do
    options = Tipi::Cli::Options.parse! %w(-d article test/fixtures/sample.tipi)
    assert_equal 'article', options[:attributes]['doctype']
  end

  test 'book doctype assignment' do
    options = Tipi::Cli::Options.parse! %w(-d book test/fixtures/sample.tipi)
    assert_equal 'book', options[:attributes]['doctype']
  end

  test 'inline doctype assignment' do
    options = Tipi::Cli::Options.parse! %w(-d inline test/fixtures/sample.tipi)
    assert_equal 'inline', options[:attributes]['doctype']
  end

  test 'template engine assignment' do
    options = Tipi::Cli::Options.parse! %w(-E haml test/fixtures/sample.tipi)
    assert_equal 'haml', options[:template_engine]
  end

  test 'template directory assignment' do
    options = Tipi::Cli::Options.parse! %w(-T custom-backend test/fixtures/sample.tipi)
    assert_equal ['custom-backend'], options[:template_dirs]
  end

  test 'multiple template directory assignments' do
    options = Tipi::Cli::Options.parse! %w(-T custom-backend -T custom-backend-hacks test/fixtures/sample.tipi)
  ...
  test 'multiple -r flags requires specified libraries' do
    options = Tipi::Cli::Options.new
    redirect_streams do |_, stderr|
      exitval = options.parse! %w(-r foobar -r foobaz test/fixtures/sample.tipi)
      assert_match %(tipi: FAILED: 'foobar' could not be loaded), stderr.string
      assert_equal 1, exitval
  ...
  test '-r flag with multiple values separated by commas is treated as a single path' do
    options = Tipi::Cli::Options.new
    redirect_streams do |_, stderr|
      exitval = options.parse! %w(-r /no-such-folder/a,b,c/ext.rb test/fixtures/sample.tipi)
      assert_include %(tipi: FAILED: '/no-such-folder/a,b,c/ext.rb' could not be loaded), stderr.string
      assert_equal 1, exitval
  ...
  test '-I option appends paths to $LOAD_PATH' do
    ...
    old_load_path = $:.dup
    begin
      exitval = options.parse! %w(-I foobar -I foobaz test/fixtures/sample.tipi)
  ...
  test '-I option appends multiple paths to $LOAD_PATH' do
    ...
    old_load_path = $:.dup
    begin
      exitval = options.parse! %W(-I foobar#{File::PATH_SEPARATOR}foobaz test/fixtures/sample.tipi)
      refute_equal 1, exitval
      assert_equal old_load_path.size + 2, $:.size
      assert_equal File.expand_path('foobar'), $:[0]
      assert_equal File.expand_path('foobaz'), $:[1]
  ...
  test 'should set failure level to FATAL by default' do
    options = Tipi::Cli::Options.parse! %w(test/fixtures/sample.tipi)
  ...
  test 'should allow failure level to be set to FATAL using any recognized abbreviation' do
    %w(f fatal FATAL).each do |val|
      options = Tipi::Cli::Options.parse! %W(--failure-level=#{val} test/fixtures/sample.tipi)
      assert_equal Logger::Severity::FATAL, options[:failure_level]
    end
  end
  ...
  test 'should not allow failure level to be set to unknown value' do
    exit_code, messages = redirect_streams do |_, err|
      [(Tipi::Cli::Options.parse! %w(--failure-level=foobar test/fixtures/sample.tipi)), err.string]
    end
  ...
  test 'should set log level to DEBUG when --log-level option is specified' do
    options = Tipi::Cli::Options.parse! %w(--log-level debug test/fixtures/sample.tipi)
  ...
  test 'should enable sourcemap when --sourcemap flag is specified' do
    options = Tipi::Cli::Options.parse! %w(--sourcemap test/fixtures/sample.tipi)
