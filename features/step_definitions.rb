...
TIPI_FEATURES_DIR = File.absolute_path __dir__
TIPI_LIB_DIR = ENV['TIPI_LIB_DIR'] || File.join(TIPI_FEATURES_DIR, '../lib')
...
Given %r/the Tipi source/ do |source|
  @source = source
end

When %r/it is converted to html/ do
  @output = Tipi.convert @source
end

When %r/it is converted to docbook/ do
  @output = Tipi.convert @source, backend: :docbook
end
