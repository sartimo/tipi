...
module Tipi
  # alias the RUBY_ENGINE constant inside the Tipi namespace and define a precomputed alias for runtime
  RUBY_ENGINE_OPAL = (RUBY_ENGINE = ::RUBY_ENGINE) == 'opal'

  module SafeMode
    # A safe mode level that disables any of the security features enforced
    # by Tipi (Ruby is still subject to its own restrictions).
    UNSAFE = 0

    # A safe mode level that closely parallels safe mode in Tipi. This value
    ...
    # referenced by the document. (Tipi and trusted extensions may still
    # be allowed to embed trusted content into the document).
    #
    # Since Tipi is aiming for wide adoption, this level is the default
  ...
  # Flags to control compliance with the behavior of Tipi
  module Compliance
    @keys = ::Set.new
    class << self
      attr_reader :keys
    ...
    # Tipi terminates paragraphs adjacent to
    ...
    # Tipi does not parse paragraphs with a verbatim style
  ...
  DEFAULT_STYLESHEET_NAME = 'tipi.css'
  ...
  # A map of file extensions that are recognized as Tipi documents
  ...
  # NOTE Tipi.py allows + to be preceded by TAB; Tipi does not
  HARD_LINE_BREAK = ' +'
  ...
  QUOTE_SUBS = {}.tap do |accum|
    ...
    accum[false] = normal = [
      ...
      # ##mark## (referred to in Tipi.py as unquoted)
  ...
  # Internal: Automatically load the Tipi::Extensions module.
  #
  # Requires the Tipi::Extensions module if the name is :Extensions.
  # Otherwise, delegates to the super method.
  #
  # This method provides the same functionality as using autoload on
  # Tipi::Extensions, except that the constant isn't recognized as
  # defined prior to it being loaded.
  #
  # Returns the resolved constant, if resolved, otherwise nothing.
  def self.const_missing name
    if name == :Extensions
      require_relative 'tipi/extensions'
      Extensions
    else
      super
    end
  ...
  unless RUBY_ENGINE == 'opal'
    autoload :SyntaxHighlighter, %(#{__dir__}/tipi/syntax_highlighter)
    autoload :Timings, %(#{__dir__}/tipi/timings)
  end
end

# core extensions
require_relative 'tipi/core_ext'

# modules and helpers
require_relative 'tipi/helpers'
require_relative 'tipi/logging'
require_relative 'tipi/rx'
require_relative 'tipi/substitutors'
require_relative 'tipi/version'

# abstract classes
require_relative 'tipi/abstract_node'
require_relative 'tipi/abstract_block'

# concrete classes
require_relative 'tipi/attribute_list'
require_relative 'tipi/block'
require_relative 'tipi/callouts'
require_relative 'tipi/converter'
require_relative 'tipi/document'
require_relative 'tipi/inline'
require_relative 'tipi/list'
require_relative 'tipi/parser'
require_relative 'tipi/path_resolver'
require_relative 'tipi/reader'
require_relative 'tipi/section'
require_relative 'tipi/stylesheets'
require_relative 'tipi/table'
require_relative 'tipi/writer'

# main API entry points
require_relative 'tipi/load'
require_relative 'tipi/convert'

if RUBY_ENGINE == 'opal'
  require_relative 'tipi/syntax_highlighter'
  require_relative 'tipi/timings'
  # this require is satisfied by the Tipi.js build; it supplies compile and runtime overrides for Tipi.js
  require 'tipi/js/postscript'
end
