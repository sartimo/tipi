...
module Tipi
# Public: The Document class represents a parsed Tipi document.
#
# Document is the root node of a parsed Tipi document. It provides an
# abstract syntax tree (AST) that represents the structure of the Tipi
...
# commonly, you'll load the document object from Tipi source using the
# primary API methods, {Tipi.load} or {Tipi.load_file}. When
# using one of these APIs, you almost always want to set the safe mode to
# :safe (or :unsafe) to enable all of Tipi's features.
#
#   Tipi.load '= Hello, Tipi!', safe: :safe
#   # => Tipi::Document { doctype: "article", doctitle: "Hello, Tipi!", blocks: 0 }
#
...
#   document = Tipi.load source, safe: :safe
#   document.doctitle
#   # => 'Document Title'
#
...
#
#   Tipi.load('no doctitle', safe: :safe).doctitle use_fallback: true
#   # => "Untitled"
#
...
#   [sidebar]
#   sidebar contents'
#   doc = Tipi.load source, safe: :safe
...
class Document < AbstractBlock
  ...
  #
  # A value of 1 (SAFE) closely parallels safe mode in Tipi. In particular,
  ...
  # document. (Tipi and trusted extensions may still be allowed to embed
  # trusted content into the document).
  #
  # Since Tipi is aiming for wide adoption, 20 (SECURE) is the default
  ...
  # Public: Get the Boolean Tipi compatibility mode
  #
  ...
  #
  # data    - The Tipi source data as a String or String Array. (default: nil)
  ...
  def initialize data = nil, options = {}
    ...
    if (parent_doc = options.delete :parent)
      ...
      initialize_extensions = (defined? ::Tipi::Extensions) || (options.key? :extensions) ? ::Tipi::Extensions : nil
    ...
    unless parent_doc
      ...
      # load attributes from data/locale/attributes-<lang>.tipi
    end
    ...
    attr_overrides['tipi'] = ''
    attr_overrides['tipi-version'] = ::Tipi::VERSION

    attr_overrides['safe-mode-name'] = (safe_mode_name = SafeMode.name_for_value @safe)
    attr_overrides[%(safe-mode-#{safe_mode_name})] = ''
    attr_overrides['safe-mode-level'] = @safe

    # the only way to set the max-include-depth attribute is via the API; default to 64 like Tipi.py
    ...
    if parent_doc
      ...
      if initialize_extensions
        if (ext_registry = options[:extension_registry])
          ...
          if Extensions::Registry === ext_registry || ((defined? ::TipiJ::Extensions::ExtensionRegistry) &&
              ::TipiJ::Extensions::ExtensionRegistry === ext_registry)
  ...
  # Public: Parse the Tipi source stored in the {Reader} into an abstract syntax tree.
  #
  ...
  #
  # data - The optional replacement Tipi source data as a String or String Array. (default: nil)
  #
  ...
  # Public: Convert the Tipi document using the templates
  ...
  def content
    # NOTE per Tipi-spec, remove the title before converting the body
  ...
  def save_attributes
    ...
    unless @parent_document
      if (basebackend = attrs['basebackend']) == 'html'
        # QUESTION should we allow source-highlighter to be disabled in Tipi table cell?
  ...
  def fill_datetime_attributes attrs, input_mtime
    ...
    # %Z is OS dependent and may contain characters that aren't UTF-8 encoded (see tipi#2770 and tipi.js#23)
  ...
  def update_backend_attributes new_backend, init = nil
    ...
    if Converter::BackendTraits === (converter = create_converter new_backend, delegate_backend)
      ...
      raise ::NotImplementedError, %(tipi: FAILED: missing converter for backend '#{new_backend}'. Processing aborted.)
    end
    @converter = converter
