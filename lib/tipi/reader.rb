...
module Tipi
# Public: Methods for retrieving lines from Tipi source files
class Reader
  include Logging
  ...
  # cleaning is very important to how Tipi works). Subclasses may choose to perform additional preparation.
  #
...
# Public: Methods for retrieving lines from Tipi source files, evaluating preprocessor
...
class PreprocessorReader < Reader
  ...
  def push_include data, file = nil, path = nil, lineno = 1, attributes = {}
    ...
    if (@file = file)
      ...
      # only process lines in Tipi files
      ...
      @dir = '.'
      # we don't know what file type we have, so assume Tipi
      @process_lines = true
      if (@path = path)
        # NOTE registering the include with a nil value tracks it while not making it visible to interdocument xrefs
  ...
  def prepare_lines data, opts = {}
    ...
    # QUESTION should this work for Tipi table cell content? Currently it does not.
  ...
  #
  # This method is overridden in Tipi.js to resolve the target of an include in the browser
  # environment.
  #
