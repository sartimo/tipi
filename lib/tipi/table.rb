...
module Tipi
# Public: Methods and constants for managing Tipi table content in a document.
# It supports all three of Tipi's table formats: psv, dsv and csv.
...
# Public: Methods to manage the columns of an Tipi table. In particular, it
...
# Public: Methods for managing the a cell in an Tipi table.
class Table::Cell < AbstractBlock
  ...
  # Public: Returns the nested Document in an Tipi table cell (only set when style is :tipi)
  ...
  def initialize column, cell_text, attributes = {}, opts = {}
    ...
    if column
      if (in_header_row = column.table.header_row?)
        if in_header_row == :implicit && (cell_style = column.style || (attributes && attributes['style']))
          @reinitialize_args = [column, cell_text, attributes && attributes.merge, opts] if cell_style == :tipi || cell_style == :literal
          cell_style = nil
        end
      else
    ...
    if attributes
      ...
      when :literal
        ...
        # QUESTION should we use same logic as :tipi cell? strip leading space if text doesn't start with newline?
        cell_text = cell_text.slice 1, cell_text.length while cell_text.start_with? LF
      else
        normal_psv = true
        # NOTE TipiJ uses nil cell_text to create an empty cell
  ...
  #
  # Used for cells in the head row as well as text-only (non-Tipi) cells in
  # the foot row and body.
  #
  # This method shouldn't be used for cells that have the Tipi style.
  #
...
# Public: Methods for managing the parsing of an Tipi table. Instances of this
...
class Table::ParserContext
  ...
  # Public: An Array of String keys that represent the table formats in Tipi
  #--
  # QUESTION should we recognize !sv as a valid format value?
  FORMATS = ::Set['psv', 'csv', 'dsv', 'tsv']

  # Public: A Hash mapping the Tipi table formats to default delimiters
  DELIMITERS = {
    'psv' => ['|', /\|/],
    'csv' => [',', /,/],
    'dsv' => [':', /:/],
  ...
  # Public: The Tipi table format (psv, dsv, or csv)
