...
module Helpers
  ...
  def require_library name, gem_name = true, on_failure = :abort
    ...
    if gem_name
      ...
      when :abort
        ...
        raise ::LoadError, %(tipi: FAILED: required gem '#{gem_name}' is not available#{details}. Processing aborted.)
      when :warn
        details = $!.path == gem_name ? '' : %[ (reason: #{$!.path ? %(cannot load '#{$!.path}') : $!.message})]
      ...
      case on_failure
      when :abort
        raise ::LoadError, %(tipi: FAILED: #{$!.message.chomp '.'}. Processing aborted.)
      when :warn
  ...
  #
  # Uses the Tipi::UriSniffRx regex to check whether the String begins
  ...
  # Examples
  #
  #   Helpers.rootname 'part1/chapter1.tipi'
  #   # => "part1/chapter1"
  #
