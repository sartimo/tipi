...
# by the docbook45 backend from Tipi.py, except it has been migrated to the DocBook 5 specification.
class Converter::DocBook5Converter < Converter::Base
  ...
  def convert_document node
    ...
    result << ((node.attr? 'toclevels') ? %(<?tipi-toc maxdepth="#{node.attr 'toclevels'}"?>) : '<?tipi-toc?>') if node.attr? 'toc'
    result << ((node.attr? 'sectnumlevels') ? %(<?tipi-numbered maxdepth="#{node.attr 'sectnumlevels'}"?>) : '<?tipi-numbered?>') if node.attr? 'sectnums'
...
#{node.content}
  ...
  def convert_page_break node
    '<simpara><?tipi-pagebreak?></simpara>'
  end
