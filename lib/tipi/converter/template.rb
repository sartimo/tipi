...
# {AbstractNode} objects from a parsed Tipi document tree to the backend
# format.
#
...
# convert a paragraph {Block} object from the parsed Tipi tree to an HTML
...
class Converter::TemplateConverter < Converter::Base
  DEFAULT_ENGINE_OPTIONS = {
    ...
    # NOTE Tipi syntax expects HTML/XML output to use double quotes around attribute values
      ...
      unless template_cache && (template = template_cache[file])
        ...
        when :slim
          unless @active_engines[extsym]
            ...
            ::Slim::Engine.define_options tipi: {}
            # align safe mode of Tipi embedded in Slim template with safe mode of current document
            # NOTE safe mode won't get updated if using template cache and changing safe mode
            (@engine_options[extsym][:tipi] ||= {})[:safe] ||= @safe if @safe
