...
class AbstractBlock < AbstractNode
  ...
  # Public: Gets/Sets the location in the Tipi source where this block begins.
  ...
  #   #=> Tipi::Section@14459860 { level: 0, title: "Hello, Tipi!", blocks: 0 }
  #   #=> Tipi::Section@14505460 { level: 1, title: "First Section", blocks: 1 }
  #
  #   doc.find_by(context: :section) {|section| section.level == 1 }
  #   #=> Tipi::Section@14505460 { level: 1, title: "First Section", blocks: 1 }
  #
  #   doc.find_by context: :listing, style: 'source'
  #   #=> Tipi::Block@13136720 { context: :listing, content_model: :verbatim, style: "source", lines: 1 }
  #
      ...
      if selector[:traverse_documents]
        ...
        (rows.body + rows.foot).each do |r|
          r.each do |c|
            ...
            c.inner_document.find_by_internal selector, result, &block if c.style == :tipi
          end
        end
      else
