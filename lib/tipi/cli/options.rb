    ...
    class Options < ::Hash
      ...
      def parse! args
        opts_parser = ::OptionParser.new do |opts|
          ...
          Usage: tipi [OPTION]... FILE...
          Convert the Tipi input FILE(s) to the backend output format (e.g., HTML 5, DocBook 5, etc.)
          ...
          opts.on '--safe',
            ...
            'provided for compatibility with the tipi command' do
          ...
          opts.on_tail '-h', '--help [TOPIC]', 'print a help message',
            ...
            'show an overview of the Tipi syntax if TOPIC is syntax',
            'dump the Tipi man page (in troff/groff format) if TOPIC is manpage' do |topic|
            case topic
            # use `tipi -h manpage | man -l -` to view with man pager
            when 'manpage'
              if (manpage_path = ::ENV['TIPI_MANPAGE_PATH'])
                if ::File.exist? manpage_path
                  ...
                  $stderr.puts %(tipi: FAILED: manual page not found: #{manpage_path})
                  return 1
                end
              # Ruby 2.3 requires the extra brackets around the ::File.join method call
              elsif ::File.exist? (manpage_path = (::File.join ROOT_DIR, 'man', 'tipi.1'))
                $stdout.puts ::File.read manpage_path
              else
                manpage_path = %x(man -w tipi).chop rescue ''
                if manpage_path.empty?
                  $stderr.puts 'tipi: FAILED: manual page not found; try `man tipi`'
                  return 1
            ...
            when 'syntax'
              # Ruby 2.3 requires the extra brackets around the ::File.join method call
              if ::File.exist? (syntax_path = (::File.join ROOT_DIR, 'data', 'reference', 'syntax.tipi'))
                $stdout.puts ::File.read syntax_path
              else
                $stderr.puts 'tipi: FAILED: syntax page not found; visit https://tipi.org/docs'
                return 1
              end
            else
              $stdout.puts opts
        ...
        if args.size == 1 && args[0] == '-'
          ...
          args.each do |file|
            if file.start_with? '-'
              ...
              $stderr.puts %(tipi: WARNING: extra arguments detected (unparsed arguments: '#{args.join "', '"}') or incorrect usage of stdin)
          ...
          begin # rubocop:disable Style/RedundantBegin
            ...
            if fstat.file? || fstat.pipe?
              unless fstat.readable?
                $stderr.puts %(tipi: FAILED: input file #{file} is not readable)
                return 1
              end
            else
              $stderr.puts %(tipi: FAILED: input path #{file} is a #{fstat.ftype}, not a file)
              return 1
            end
          rescue ::Errno::ENOENT
            $stderr.puts %(tipi: FAILED: input file #{file} is missing)
            return 1
          end
        end
        ...
        if self[:template_dirs]
          begin
            ...
            raise $! if self[:trace]
            $stderr.puts 'tipi: FAILED: \'tilt\' could not be loaded'
        ...
        if (requires = self[:requires])
          ...
          requires.each do |path|
            begin # rubocop:disable Style/RedundantBegin
              ...
              raise $! if self[:trace]
              $stderr.puts %(tipi: FAILED: '#{path}' could not be loaded)
              $stderr.puts '  Use --trace to show backtrace'
              return 1
            rescue ::SystemExit
              # not permitted here
        ...
        $stderr.puts %(tipi: option #{$!.message})
        $stdout.puts opts_parser
        1
      rescue ::OptionParser::InvalidOption, ::OptionParser::InvalidArgument
        $stderr.puts %(tipi: #{$!.message})
        $stdout.puts opts_parser
        1
      ensure
        $VERBOSE = old_verbose
      ...
      def print_version os = $stdout
        os.puts %(Tipi #{::Tipi::VERSION} [https://tipi.org])
