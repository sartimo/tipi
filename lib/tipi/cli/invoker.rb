...
module Tipi
  module Cli
    # Public Invocation class for starting Tipi via CLI
    class Invoker
      include Logging
      ...
      def invoke!
        ...
        if stdin
          ...
          if show_timings
            @documents << (::Tipi.convert input, (input_opts.merge timings: (timings = Timings.new)))
            timings.print_report err, '-'
          else
            @documents << (::Tipi.convert input, input_opts)
          end
        else
          infiles.each do |infile|
            ...
            if show_timings
              @documents << (::Tipi.convert_file infile, (input_opts.merge timings: (timings = Timings.new)))
              timings.print_report err, infile
            else
              @documents << (::Tipi.convert_file infile, input_opts)
            end
          end
        end
