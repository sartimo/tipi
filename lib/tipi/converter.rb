...
module Tipi
# A module for defining converters that are used to convert {AbstractNode} objects in a parsed Tipi document to an
# output (aka backend) format such as HTML or DocBook.
#
# A {Converter} is typically instantiated each time an Tipi document is processed (i.e., parsed and converted).
...
#     end
#   end
#   puts Tipi.convert_file 'sample.tipi', backend: :text, safe: :safe
#
#   class Html5Converter < (Tipi::Converter.for 'html5')
#     register_for 'html5'
#     def convert_paragraph node
#       %(<p>#{node.content}</p>)
#     end
#   end
#   puts Tipi.convert_file 'sample.tipi', safe: :safe
module Converter
  ...
  # Tipi document to a backend format such as HTML or DocBook.
  class Base
    include Logging
    include Converter
