...
module Tipi
# Public: Methods to perform substitutions on lines of Tipi text. This module
...
module Substitutors
    ...
    if ((text.include? '&') && (text.include? ';&l')) || (found_macroish && (text.include? 'xref:'))
      text = text.gsub InlineXrefMacroRx do
        ...
        if target
          ...
          # handles: path#, path#id, path.tipi#, path.tipi#id, or path.tipi (xref macro only)
  ...
  def extract_passthroughs text
    ...
    text = text.gsub InlineStemMacroRx do
      ...
      # NOTE drop enclosing $ signs around latexmath for backwards compatibility with Tipi.py
  ...
  def commit_subs
    unless (default_subs = @default_subs)
      ...
      when :raw
        # TODO make pass subs a compliance setting; Tipi.py performs :attributes and :macros on a pass block
